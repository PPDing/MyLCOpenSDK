...
class LCOpenApiResponse;
   ...
   private:
    ...
    public:
        ...
        static LCOpenApiHandle client_create(protocolType protocol, const char *host, int port, const char* accessSource = "lc");
        static void client_destroy(LCOpenApiHandle hc);
		static std::string client_get_access_source(LCOpenApiHandle hc);
		bool setListener(void *lis);
		void *getListener();
        int request(LCOpenApiHandle hc, LCOpenApiRequest* req, LCOpenApiResponse* resp, int timeout);
		int requestEx(LCOpenApiHandle hc, char *url, char *method, char *contentType, char *header, char *body, int isUseGzip,char **rspCode,char **rspMsg, char **rspHeaders, char **rspBody , int timeout, int isUseAlive, int nReqIndex, int nPriority);
        ...
        void set_dolynk_deviceId(const string& deviceId);
        //for new cos
        
        void set_phoneId(const string& phoneId);
        void set_appVersion(const string& version);
...
//        const string get_auth_password();
        ...
        const string get_dolynk_deviceId();
        
        //for new cos
        const string get_phoneId();
        const string get_appVersion();
        
        bool initLongConnection(const char* jsonParam);
    ...
    private:
		...
		void *m_listener;
        //
        typedef struct _ServerHostParams {
            std::string host;
            int port;
            bool isHttps;
            std::string sourceType;
            std::string token;
			bool isDetectConnection;
        }ServerHostParams;
        std::map<std::string, ServerHostParams> m_backupServerHosts;
    public:
       int      logLevel;
