...
///@brief 事件类型EVENT_IVS_FACERECOGNITION(目标识别)对应的数据块描述信息
typedef struct tagDEV_EVENT_FACERECOGNITION_INFO_V2
{
	int                 nChannelID;                                 // 通道号
	char                szName[128];                                // 事件名称
	int                 nEventID;                                   // 事件ID
	...
	char				szReserved[10240];							// 保留字节
}DEV_EVENT_FACERECOGNITION_INFO_V2;
...
typedef struct tagNET_IN_SET_IVSEVENT_PARSE_INFO
{
    ...
    DWORD                           dwIVSEvent;                                 // 事件类型,参考dhnetsdk.h中定义的智能分析事件宏或报警事件宏.
																				...
																				//															dwStructType为 2 时,对应结构体 DEV_EVENT_FACERECOGNITION_INFO_V2
                                                                                // nCallBackType为1时(即报警事件解析类型)：
                                                                                // DH_ALARM_RTSC_RUNING     :       dwStructType为 1 时,对应结构体 ALARM_RTSC_RUNNING_INFO_V1
																				// nCallBackType为2时(即智能任务事件解析类型)：
																				// EM_ANALYSE_EVENT_FACE_RECOGNITION（传对应int类型的值即可）     :       dwStructType为 2 时,对应结构体 DEV_EVENT_FACERECOGNITION_INFO_V2
...
typedef enum tagEM_ATTACH_SECOND_ALARM_PROTOCOL
{
	...
	EM_DH_ALARM_ALARM_EX = 1,						// 外部报警与远程外部报警
	EM_DH_MOTION_ALARM_EX = 2,						// 动态检测报警
	EM_DH_VIDEOLOST_ALARM_EX = 3,					// 视频丢失报警
	EM_DH_SHELTER_ALARM_EX = 4,						// 视频遮挡报警
	EM_DH_SOUND_DETECT_ALARM_EX = 5,				// 音频检测报警
	EM_DH_DISKFULL_ALARM_EX = 6,					// 硬盘满报警
	EM_DH_DISKERROR_ALARM_EX = 7,					// 坏硬盘报警
	EM_DH_ENCODER_ALARM_EX = 10,					// 编码器报警
	EM_DH_URGENCY_ALARM_EX = 11,					// 紧急报警
	EM_DH_WIRELESS_ALARM_EX = 12,					// 无线报警
	EM_DH_DECODER_DECODE_ABILITY = 13,				// 解码器能力不足报警
	EM_DH_FDDI_DECODER_ABILITY = 14,				// 光纤编码器状态报警
	EM_DH_PANORAMA_SWITCH_ALARM_EX = 15,			// 切换场景报警
	EM_DH_LOSTFOCUS_ALARM_EX = 16,					// 失去焦点报警
	EM_DH_DSP_ALARM_EX = 17,						// DSP报警
	EM_DH_RECORD_CHANGED_EX = 18,					// 录像状态变化报警
	EM_DH_DEVICE_REBOOT_EX = 19,					// 设备重启报警
	EM_DH_CONFIG_CHANGED_EX = 20,					// 配置发生变化报警
	EM_DH_STATIC_ALARM_EX = 22,						// 静态视频检测报警
	EM_DH_PTZ_LOCATION_EX = 23,						// 云台定位信息报警
	EM_DH_WINGDING_ALARM_EX = 25,					// 线圈/车检器报警
	EM_DH_TRAF_CONGESTION_ALARM_EX = 26,			// 交通阻塞报警
	EM_DH_TRAF_EXCEPTION_ALARM_EX = 27,				// 交通异常报警
	EM_DH_EQUIPMENT_FILL_ALARM_EX = 28,				// 补光设备故障报警
	EM_DH_ALARM_ACC_POWEROFF = 30,					// ACC断电报警
	EM_DH_ALARM_3GFLOW_EXCEED = 32,					// 3G流量超出阈值报警
	EM_DH_ALARM_SPEED_LIMIT = 33,					// 对应二代报警中的 DH_ALARM_SPEED_LIMIT, 限速报警
	EM_DH_ALARM_VEHICLE_INFO_UPLOAD = 34,			// 车载定制信息上传
	EM_DH_ALARM_CARD_RECORD_UPLOAD = 35,			// 卡号录像卡号信息上传
	EM_DH_ALARM_ATM_INFO_UPLOAD = 36,				// ATM交易信息上传
	EM_DH_ALARM_DETAILEDMOTION = 37,				// 动态检测详细事件信息上传
	EM_DH_ALARM_FRONTDISCONNECT = 38,				// 前端断网报警
	EM_DH_ALARM_BATTERYLOWPOWER = 39,				// 电池电压过低报警
	EM_DH_ALARM_TEMPERATURE = 40,					// 温度过高报警
	EM_DH_ALARM_TIREDDRIVE = 41,					// 疲劳驾驶报警
	EM_DH_LONG_TIME_NO_OPERATION = 42,				// WEB用户长时间无操作事件
	EM_DH_BLACKLIST_SNAP = 43,						// 禁止名单车牌抓拍事件
	EM_DH_ALARM_DECODER_ALARM_EX = 156,				// 报警解码器报警
	EM_DH_OEMSTATE_EX = 161,						// Oem需求事件类型
	EM_DH_ATMPOS_BROKEN_EX = 162,					// Atm和pos机断开报警
	EM_DH_ALARM_ARM_DISARM_STATE = 164,				// 报警布撤防状态
	EM_DH_ALARM_ENCLOSURE = 165,					// 对应二代报警中的 DH_ALARM_ENCLOSURE, 电子围栏报警
	EM_DH_ALARM_SIP_STATE = 166,					// SIP状态变化通知
	EM_DH_ALARM_RAID_STATE = 167,					// Raid状态变化通知
	EM_DH_ALARM_OVER_LOADING = 168,					// 超载报警
	EM_DH_ALARM_HARD_BRAKING = 169,					// 急刹车报警
	EM_DH_ALARM_CROSSING_SPEED_LIMIT = 170,			// 路口限速报警
	EM_DH_ALARM_TRAFFIC_LIGHT_FAULT = 171,			// 交通灯故障报警
	EM_DH_ALARM_SMOKE_SENSOR = 172,					// 火警报警
	EM_DH_ALARM_TRAFFIC_FLUX_STAT = 173,			// 交通流量报警事件
	EM_DH_ALARM_CAMERA_MOVE = 174,					// 摄像机移位事件
	EM_DH_ALARM_STORAGE_FAILURE = 175,				// 录像异常事件
	EM_DH_ALARM_LOST_RECORD = 176,					// 丢录像事件
	EM_DH_ALARM_HIGH_CPU = 177,						// CPU使用率过高事件
	EM_DH_ALARM_LOST_NETPACKET = 178,				// 网络发送数据丢包事件
	EM_DH_ALARM_HIGH_MEMORY = 179,					// 内存占有率过高
	EM_DH_ALARM_DISK = 180,							// 硬盘告警详细信息
	EM_DH_ALARM_FILE_SYSTEM = 181,					// 文件系统告警
	EM_DH_ALARM_IVS = 182,							// 智能检测事件
	EM_DH_ALARM_DISKBURNED_FULL = 183,				// 刻录已满
	EM_DH_ALARM_POWER_ABNORMAL = 189,				// 冗余电源异常报警
	EM_DH_ALARM_NO_DISK = 190,						// 无硬盘报警
	EM_DH_ALARM_PROTECTIVE_CAPSULE = 194,			// 防护舱检测事件
	EM_DH_ALARM_3RD_PROTOCAL = 200,					// 三代事件格式订阅
	EM_DH_ALARM_CONFIG_ENABLE_CHANGE = 202,			// 配置使能改变上报事件
	EM_DH_ALARM_ITC_HWS000 = 203,					// 虚点测速仪设备事件与报警
...
typedef struct tagNET_IN_PROBE_ALARM
{
	...
	EM_ATTACH_SECOND_ALARM_PROTOCOL					emAttachEventType[128];				// 需要订阅的二代报警，支持的订阅报警类型见EM_ATTACH_SECOND_ALARM_PROTOCOL,目前仅支持两个事件
}NET_IN_PROBE_ALARM;
...
///@brief 回调的实时数据
typedef struct tagNET_NOTIFY_IOTBOX_COMM_EX
{
	char									*pszOutBuf;						/// 回调的数据json内容
	int										nOutBufLen;						/// pszOutBuf返回内容大小
	char									szResvered[252];				/// 保留字段
}NET_NOTIFY_IOTBOX_COMM_EX;

///@brief 订阅485实时数据接口回调函数原型, lAttachHandle 为 CLIENT_AttachIotboxCommEx 接口的返回值
///@param[out] lAttachHandle 订阅句柄
///@param[out] pszOutBuf 订阅的485实时数据
///@param[out] bufSize 数据长度
///@param[out] dwUser 用户信息
///@return void
typedef void (CALLBACK *fNotifyIotboxRealdataEx)(LLONG lAttachHandle, NET_NOTIFY_IOTBOX_COMM_EX* pstuIotboxCommEx, LDWORD dwUser);

///@brief CLIENT_AttachIotboxCommEx 接口入参
typedef struct tagNET_IN_ATTACH_IOTBOX_COMM_EX
{
	DWORD									dwSize; 								/// 此结构体大小,必须赋值
	LDWORD									dwUser; 								/// 用户自定义参数
	fNotifyIotboxRealdataEx					cbNotifyIotboxRealdataEx; 				/// 回调函数
}NET_IN_ATTACH_IOTBOX_COMM_EX;

///@brief CLIENT_AttachIotboxCommEx 接口出参
typedef struct tagNET_OUT_ATTACH_IOTBOX_COMM_EX
{
	DWORD									dwSize; 								/// 此结构体大小,必须赋值
}NET_OUT_ATTACH_IOTBOX_COMM_EX;

///@brief 订阅485实时数据扩展
///@param[in] lLoginID 登录句柄
///@param[in] pstuInParam 接口输入参数 
///@param[out] pstuOutParam 接口输出参数
///@param[in] nWaitTime 接口超时时间, 单位毫秒
///@return 返回订阅句柄
CLIENT_NET_API LLONG CALL_METHOD CLIENT_AttachIotboxCommEx(LLONG lLoginID,const NET_IN_ATTACH_IOTBOX_COMM_EX* pstuInParam, NET_OUT_ATTACH_IOTBOX_COMM_EX* pstuOutParam, int nWaitTime);

///@brief 取消订阅实时数据扩展
///@param[in] lAttachHandle 订阅句柄
///@return TRUE表示成功 FALSE表示失败
CLIENT_NET_API BOOL CALL_METHOD CLIENT_DetachIotboxCommEx(LLONG lAttachHandle);

///@brief 回调的实时数据
typedef struct tagNET_VIDEO_ANALYSE_ANALYSE_PROC_EX
{
	char									*pszOutBuf; 					/// 回调的数据json内容
	int 									nOutBufLen; 					/// pszOutBuf返回内容大小
	char									szResvered[252];				/// 保留字段
}NET_VIDEO_ANALYSE_ANALYSE_PROC_EX;

///@brief 智能分析结果的回调函数
///@param[out] lAttachHandle 订阅句柄
///@param[out] pstuVideoAnalyseTrackProc 智能分析结果的信息
///@param[out] dwUser 用户信息
///@return void
typedef void (CALLBACK *fVideoAnalyseAnalyseProcEx)(LLONG lAttachHandle, NET_VIDEO_ANALYSE_ANALYSE_PROC_EX* pstuVideoAnalyseTrackProcEx, LDWORD dwUser);

///@brief CLIENT_AttachVideoAnalyseAnalyseProc 输入参数
typedef struct tagNET_IN_ATTACH_VIDEO_ANALYSE_ANALYSE_PROC_EX
{
	DWORD						dwSize;						/// 此结构体大小,必须赋值
	int							nChannelId;					/// 通道号
	fVideoAnalyseAnalyseProcEx	cbVideoAnalyseAnalyseProcEx;	/// 回调函数
	LDWORD						dwUser;						/// 用户信息
}NET_IN_ATTACH_VIDEO_ANALYSE_ANALYSE_PROC_EX;

///@brief CLIENT_AttachVideoAnalyseAnalyseProc 输出参数
typedef struct tagNET_OUT_ATTACH_VIDEO_ANALYSE_ANALYSE_PROC_EX
{
	DWORD						dwSize;						/// 此结构体大小,必须赋值
}NET_OUT_ATTACH_VIDEO_ANALYSE_ANALYSE_PROC_EX;

///@brief 订阅智能分析结果
///@param[in] 	lLoginID 登录句柄
///@param[in] 	pstInParam 接口输入参数 
///@param[out]	pstOutParam 接口输出参数 
///@param[in] 	nWaitTime 接口超时时间, 单位毫秒
///@return 返回订阅句柄
CLIENT_NET_API LLONG CALL_METHOD CLIENT_AttachVideoAnalyseAnalyseProcEx(LLONG lLoginID, NET_IN_ATTACH_VIDEO_ANALYSE_ANALYSE_PROC_EX* pstInParam, NET_OUT_ATTACH_VIDEO_ANALYSE_ANALYSE_PROC_EX* pstOutParam , int nWaitTime);

///@brief 取消订阅智能分析结果
///@param[in] 	lAttachHandle 订阅句柄
///@return TRUE表示成功 FALSE表示失败
CLIENT_NET_API BOOL CALL_METHOD CLIENT_DetachVideoAnalyseAnalyseProcEx(LLONG lAttachHandle);
