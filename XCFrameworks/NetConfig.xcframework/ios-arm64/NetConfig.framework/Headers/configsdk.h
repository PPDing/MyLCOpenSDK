...
#define CFG_POINTERSIZE sizeof(void*)
...
#define MAX_HUMANFACE_LIST_SIZE                 8               // 视频分析设备支持的目标检测类型列表个数上限
#define MAX_FEATURE_LIST_SIZE					32				// 视频分析设备支持的目标属性列表个数上限
...
///@brief MACRO_GROUP_GET_NEW_DEV_37_BEGIN
/************************************************************************
...
#define CFG_CMD_ANALYSEGLOBAL                   "VideoAnalyseGlobal"        // 视频分析全局配置(对应 CFG_ANALYSEGLOBAL_EX_INFO), 后续CFG_ANALYSEGLOBAL_INFO不在扩展  
...
#define CFG_CMD_TRANSRADER                      "TransRadar"                // T雷达配置
#define CFG_CMD_LANDUNRADER                     "LanDunRadar"               // L雷达配置
#define CFG_CMD_LANDUNCOILS                     "LanDunCoils"               // 线圈配置
...
#define CFG_CMD_VSP_GAYS                        "VSP_GAYS"                  // 平台接入配置(CFG_VSP_GAYS_INFO)
...
#define CFG_CMD_THERMOMETRY                     "HeatImagingThermometry"    // 热成像测温全局配置(CFG_THERMOMETRY_INFO)(通道无关，通道号传-1)
...
#define CFG_CMD_VSP_SCYDKD                      "VSP_SCYDKD"                // 平台接入配置(对应 CFG_VSP_SCYDKD_INFO)
...
#define CFG_CMD_OIL_4G_OVERFLOW                 "Oil4GFlow"                 // 4G流量阈值及模式配置(对应结构体 CFG_OIL_4G_OVERFLOW_INFO)
#define CFG_CMD_OIL_VIDEOWIDGET_4G_FLOW         "VideoWidget4GFlow"         // 4G流量OSD叠加配置(对应结构体 CFG_OIL_VIDEOWIDGET_4G_FLOW_INFO)
...
#define	CFG_CMD_NETNVR_ENCRYPT                  "NvrEncrypt"                // 加密配置信息，涉及IPC和NVR( CFG_NETNVR_ENCRYPT_INFO )
...
#define CFG_CMD_SECURITY_ALARMS_PRIVACY         "SecurityAlarmsPrivacy"     // 隐私保护(对应结构体CFG_SECURITY_ALARMS_PRIVACY)
...
#define CFG_CMD_SMARTHOME_SCENELIST				"SmartHomeSceneList"		// 智能家居场景列表(对应结构体CFG_SMARTHOME_SCENELIST_INFO)
...
#define	CFG_CMD_ACCESS_OEM						"AccessOEMSettings"			// 门禁信息配置，对应结构体CFG_ACCESS_OEM_INFO
...
#define CFG_CMD_REMOTE_ANALYSEGLOBAL                   "RemoteVideoAnalyseGlobal"        // 远程视频分析全局配置(对应 CFG_ANALYSEGLOBAL_EX_INFO), 后续CFG_ANALYSEGLOBAL_INFO不在扩展
...
#define CFG_CMD_ACCESS_DOOR_STATUS				"AccessDoorStatus"			// 时间表对应门状态配置 与门禁刷卡时间段(CFG_ACCESS_TIMESCHEDULE_INFO)配套使用 对应(CFG_ACCESS_DOORSTATUS_INFO)
#define CFG_CMD_UPNP							"UPnP"						// UPnP配置(对应CFG_UPNP_INFO)
#define CFG_CMD_ESDCOMMSETTING					"ESDCommSetting"			// ESD通用配置(对应CFG_ESDCOMMSETTING_INFO)
#define CFG_CMD_VTO_CALL_LIFT_INFO				"VTOCallLiftInfo"				// 门口机梯控信息配置(对应CFG_VTO_CALL_LIFT_INFO)

///@brief MACRO_GROUP_GET_NEW_DEV_37_END

///@brief MACRO_GROUP_QUERY_CAPS_38_BEGIN
/************************************************************************
...
///@brief MACRO_GROUP_QUERY_CAPS_38_END

///@brief MACRO_GROUP_VIDEOINANALYSE_39_BEGIN
/************************************************************************
...
typedef struct tagCFG_CAP_LOG
{
	...
	BOOL		bSupportRealUTC;	// 查询条件是否支持日志查询的UTC能力
}CFG_CAP_LOG;
...
typedef enum tagEM_SUPPORTFINGERPRINT
{
	...
	EM_SUPPORTFINGERPRINT_NONSUPPORT,			// 不支持信息功能
	EM_SUPPORTFINGERPRINT_SUPPORT,				// 支持信息功能
...
typedef struct tagCFG_CAP_ACCESSCONTROL
{
	...
	EM_SUPPORTFINGERPRINT				emSupportFingerPrint;			// 是否支持信息功能
	...
	UINT								nSupportPersonCollection;		// 能力bit位控制：0-不支持人员采集新接口, 1-支持人脸使用新接口, 2-支持身份证采集使用新接口, 4-支持指纹采集使用新接口, 8-支持卡采集使用新接口 16-支持虹膜采集使用新接口, 32-支持掌纹采集使用新接口 
	BOOL								bSupportSpecialDaysConfig;		// 是否支持平台下发假日计划
	BOOL								bSupportSetDoorChannelSensorType;		// 是否支持按照门通道设置门磁的常开/常闭类型
	BOOL								bSupportAccessControlSnap;		// 楼宇设备是否支持开门抓图并上传功能配置
	UINT								nCardSetFunction;				// IC卡设置相关能力bit控制,0表示不支持，支持内容根据bit控制,卡片能力集，对应能力位：1支持，0不支持
																		// bit0: IC加密使能
																		// bit1: IC使能
																		// bit2: CPU卡使能
																		// bit3: PSAM卡使能
																		...
																		// bit5: desfire卡使能
																		// bit6: desfire卡加密使能
	BOOL								bSupportSetRepeatCardEnterTime;	// 是否支持设置卡片重复验证时间
	char								szResvered[1004];				// 保留字段
...
typedef enum tagEM_SENSE_METHOD
{
	...
	EM_SEHSE_RSUGENERAL,		// 通用模拟量
	EM_SEHSE_RSUDOOR,			// 门禁感应
	EM_SEHSE_RSUPOWEROFF,		// 断电感应
...
typedef enum tagEM_SCENE_TYPE
{
	...
	EM_SCENE_FACEDETECTION,		// "FaceDetection" 目标检测/目标识别
	EM_SCENE_ATM,				// "ATM"
	...
	EM_SCENE_SDFACEDETECTION,	// "SDFaceDetect"  多预置点目标检测，配置一条规则但可以在不同预置点下生效
	EM_SCENE_HEAT_MAP_PLAN,		// "HeatMapPlan" 球机热度图计划
	EM_SCENE_NUMBERSTAT_PLAN,	// "NumberStatPlan"	球机客流量统计计划
	EM_SCENE_ATMFD,				// "ATMFD"金融目标检测，包括正常目标、异常目标、相邻目标、头盔目标等针对ATM场景特殊优化
	...
	EM_SCENE_FACEANALYSIS,		// "FaceAnalysis" 目标分析(同时支持目标检测、目标识别、目标属性等综合型业务)
	...
	EM_SCENE_FACEATTRIBUTE,		// "FaceAttribute" IVSS目标检测
	...
	EM_SCENE_SHOPTRUCK_DETECT,				// "ShopTruckDetect" 工程车检测
	EM_SCENE_GENEAL_ATTITUDE,				// "GenealAttitude" 姿态检测
	EM_SCENE_TRAFFIC_MAINTENANCE,			/// "TrafficMaintenance" 交通运维
	EM_SCENE_TRAFFIC_THROW,					/// "TrafficThrow" 交通抛洒物品检测
	EM_SCENE_GROUND_THING,					/// "GroundThing" 地物检测
	EM_SCENE_OCR_DETECTION,					/// "OCRDetection" OCR字符识别
	EM_SCENE_OCR_MOSAIC,					/// "Mosaic" 隐私打码
	EM_SCENE_TRAFFIC_EVENT,					/// "TrafficEvent" 交通事件检测（该模式下，检测到的违章，作为事件报警使用，不做违章处罚）
	EM_SCENE_OPEN_INTELLI,					/// "OpenIntelli" 开放智能目标类检测
}EM_SCENE_TYPE;

///@brief MACRO_GROUP_PARSEDATA_CB_40_BEGIN
 /************************************************************************
...
///@brief MACRO_GROUP_PARSEDATA_CB_40_END
...
typedef struct AV_CFG_RemoteDevice 
{
	...
	char				szResvered1[2];					// 字节对齐
	BOOL				bEncryption;					// 是否采用SSL加密传输数据,暂时仅适用从HTTP切换到HTTPS
}AV_CFG_RemoteDevice;
...
typedef enum tagCFG_H264_PROFILE_RANK
{
	...
	PROFILE_HIGH,                               // 即FRExt，Main_Profile基础上新增：8x8 intra prediction(8x8 帧内预测)
...
///@brief 交通灯属性扩展
typedef struct tagCFG_LIGHTATTRIBUTE_V2
{
	BOOL             bEnable;                           // 当前交通灯是否有效，与车辆通行无关的交通需要设置无效
	int              nTypeNum; 
	char		     szLightType[MAX_LIGHT_TYPE][MAX_NAME_LEN];      // 当前交通灯显现内容（包括:红-Red,黄-Yellow,绿-Green,倒计时-Countdown），如某交通灯可以显示红黄绿三种颜色，某交通灯只显示倒计时
	...
	char             szDirection[MAX_LIGHT_DIRECTION][MAX_NAME_LEN];      // 交通灯指示的行车方向,"Straight": 直行，"TurnLeft":左转，"TurnRight":右转，"U-Turn": 掉头
	int              nYellowTime;                       // 黄灯亮时间
	UINT				nDetectorPort;					// 该灯对应的外接网络红绿灯信号检测器上的端口
	int	            nGroupType;						// 信号灯主体类型
	char				szReserved[64];					// 保留字节
}CFG_LIGHTATTRIBUTE_V2;

///@brief 交通灯组配置信息 扩展
typedef struct tagCFG_LIGHTGROUPS_V2
{
	int                nLightGroupId;                     // 灯组编号
	CFG_RECT           stuLightLocation;                  // 灯组坐标
	int                nDirection;                        // 灯组的方向,1- 灯组水平向,2- 灯组垂直向
	...
	BOOL               bSwingDetection;                   // 是否支持自适应灯组摇摆检测,在风吹或者容易震动的场景下，位置会进行一定的浮动偏差。如果由算法自行检测，会增加检测时间
	int                nLightNum;                         // 灯组中交通灯的数量
	CFG_LIGHTATTRIBUTE_V2 stuLightAtrributes[MAX_LIGHT_NUM]; // 灯组中各交通灯的属性
	char				szPassType[16];						// 通行依据"RedOff": 红灯灭 "GreenOn":绿灯亮
	char				szReserved[128];					// 保留字节
...
typedef enum EM_FACEDETECTION_TYPE
{
	...
	EM_FACEDETECTION_TYPE_SMALLFACE,						//小脸类型，目标在视频中的比重大概17%
	EM_FACEDETECTION_TYPE_LARGEFACE,						//大脸类型，目标在视频中的比重大概33%
...
typedef struct tagCFG_ENCODE_INFO
{
	...
	BOOL				bIsUseExtraStreamEx;				// 是否使用辅码流扩展
	CFG_VIDEOENC_OPT	stuExtraStreamEx[12];				// 辅码流扩展，0－辅码流1，1－辅码流2，2－辅码流3，3－辅码流4，4－辅码流5，5－辅码流6，6－辅码流7，7－辅码流8，8－辅码流9，9－辅码流10，10－辅码流11，11－辅码流12
	int                 nValidCountExtraStreamEx;            // 辅码流扩展数组中有效的个数
	char				szReserved[120];					 // 保留字节
} CFG_ENCODE_INFO;
...
typedef struct tagCFG_RECORD_INFO
{
    ...
    CFG_BACKUP_LIVE_INFO     stuBackupLiveInfo[8];          // 实时抽帧配置,EVS
	BOOL                     bSaveVideo;                    // 是否录制视频帧
	BOOL						bSaveAudio;					// 录像时是否保存音频数据
	BOOL						bEnable;					// 报警输入使能
	UINT						nMaxRecordTime;				// 单次Pir录像时长限制（单位秒），0表示无限制
...
typedef enum tagCFG_SPLITMODE
{
		...
		SPLITMODE_PIPN = 1999,					// 画中画模式，一个全屏大画面，加n个小画面窗口。这种模式下，小画面窗口的数量是可变的，但窗口数量取决于静态配置，不可动态变化。
		...
		SPLITMODE_4A = 4001,				/// 同时输出4个画面，一个大画面在左边，3个小画面在右边排成一列
		SPLITMODE_4C = 4002,				/// 同时输出1*4个通道(1行4列)，组合序号0输出0-3通道，组合序号1输出4-7通道，依次类推。C表示Column
		SPLITMODE_EOF,                          // 结束标识
} CFG_SPLITMODE;
...
typedef struct tagCFG_ALARM_MSG_HANDLE
{
    ...
    bool				abScheduleID;								// 日程ID是否有效
	short				nScheduleID;								// 日程ID
	...
	int					nAlarmOutTime;								// 报警输出持续时间,单位秒, 如果无此字段，按设备原来的方式实现
	bool				abBeepTime;									// nBeepTime 是否有效
	int					nBeepTime;									// 蜂鸣时长，单位秒，最大值为3600，0代表持续蜂鸣
...
typedef struct tagCFG_CAP_ALARM_INFO
{
	...
	BOOL				bVTHSensorTypeCorrectly;					// VTH报警输入传感器类型NO、NC正确使用，无这个能力或false的VTH表示NO、NC与协议相反使用
	char				szResvered[1024];							// 保留字段
}CFG_CAP_ALARM_INFO;
...
typedef struct tagCFG_PTZ_PER_AUTOMOVE_INFO
{
	...
	BOOL				bEnableSpecialTime;		// 是否启用szSpecialTime字段
	int					nSpecialTimeNum;		// 指定日期的工作时间段数量
	char					szSpecialTime[64][32];	// 指定日期的工作时间 如："2023-05-05 00:00:00-08:00:00"表示2023年5月5号0点到8点生效
...
///@brief 精准人数统计类型
typedef enum tagEM_RULE_TYPE_ACCURATE
{
    EM_RULE_TYPE_ACCURATE_UNKNOWN,              // 未知
    EM_RULE_TYPE_ACCURATE_NUMBERSTAT,           // 客流ReID
    EM_RULE_TYPE_ACCURATE_TARGETRECOGNITION,	// 目标去重
    EM_RULE_TYPE_ACCURATE_NUMBERSTATCLUSTER, 	// 客流聚档
}EM_RULE_TYPE_ACCURATE;

///@brief 支持的去重方向
typedef enum tagEM_DIRECTION_ACCURATE
{
    EM_DIRECTION_ACCURATE_UNKNOWN,         // 未知
    EM_DIRECTION_ACCURATE_ENTER,           // 进入
    EM_DIRECTION_ACCURATE_EXIT,			   // 离开
}EM_DIRECTION_ACCURATE;

///@brief 精准客流能力，客流ReID, 人脸去重，客流聚档的对外能力
typedef struct tagCFG_ACCURATE_NUMBER_STAT
{
	BOOL					bSupport;						// 是否支持精准客流
	BOOL					bSupportLocalDataStore;			// 是否支持本地报表查询
	EM_RULE_TYPE_ACCURATE   emRuleType;						// 精准人数统计类型
	int						nChannelsNum;					// 支持客流融合的通道号个数
	int						nChannels[1024];				// 支持客流融合的通道号，在RuleType为NumberStatCluster时有效
	int						nAreaNum;						// 单个通道支持的区域个数，NVR填写1，IPC根据支持的区域数填写
	BOOL					bSupportTimeCfg;				// 是否支持营业时间设置，FaceRecognition存在支持和不支持营业时间设置两种形态
	int 					nDirectionNum;					// 支持的去重方向个数
	EM_DIRECTION_ACCURATE   emDirection[32];				// 支持的去重方向
	int 					nGranularityNum;				// 支持查询粒度个数
	char                    szGranularity[8][8];            // 支持查询粒度
}CFG_ACCURATE_NUMBER_STAT;
...
typedef struct tagCFG_NUMBER_STAT_INFO
{
	...
	CFG_ACCURATE_NUMBER_STAT	stuAccurateNumberStat;		// 精准客流能力，客流ReID, 目标去重，客流聚档的对外能力
...
typedef enum tagCFG_EM_FACE_SNAP_POLICY
{
	...
	CFG_EM_FACE_SNAP_POLICY_QUICK,						// 快速优选，从检测到目标/人体开始，抓拍一定帧数内的质量最好的目标或人体
	CFG_EM_FACE_SNAP_POLICY_FULLTRACK,					// 全程优选，抓拍全程质量最好的目标人体
	CFG_EM_FACE_SNAP_POLICY_INTERVAL,					// 间隔抓拍
	CFG_EM_FACE_SNAP_POLICY_SINGLE,						// 单人模式，常用于门禁
	CFG_EM_FACE_SNAP_POLICY_PRECISION,					// 高精度模式，增强目标识别
}CFG_EM_FACE_SNAP_POLICY;

///@brief 人体检测能力集
typedef struct tagFACEBODY_DETECT_CAPS
{
	CFG_EM_FACE_TRAIT				szFaceFeatureList[32];			// 支持检测的目标属性
	unsigned int					nFaceFeature;					// 返回的目标属性个数
	CFG_EM_BODY_TRAIT				szBodyFeatureList[32];			// 支持检测的人体属性
	unsigned int					nBodyFeature;					// 返回的目标属性个数
	CFG_EM_FACE_SNAP_POLICY			szSnapPolicyList[16];			// 支持抓拍属性
	unsigned int					nSnapPolicy;					// 返回的目标属性个数	
...
typedef struct
{
	...
	int               	nSupportFaceTypeNum;                                       	// 支持的目标检测类型个数
	char              	szSupportFaceTypes[MAX_HUMANFACE_LIST_SIZE][MAX_NAME_LEN]; 	// 支持的目标检测类型列表
	BOOL              	bTriggerPosition;                                        	// 是否支持触发报警位置
	BOOL			  	bTriggerTrack;												// 是否支持触发跟踪
	BOOL			  	bFaceDetection;												// 是否允许配置开启目标属性识别
	int				  	nSupportFeactureNum;										// 支持的目标属性类型个数
	EM_FACEFEATURE_TYPE	emFeatureList[MAX_FEATURE_LIST_SIZE];						// 支持的目标属性类型列表
    int                 nMaxRules;                                                  // 支持规则的最大个数
	FACEBODY_DETECT_CAPS			stuFaceBodyDetectCaps;							// 人体检测能力
	FACEBODY_ANALYSE_CAPS			stuFaceBodyAnalyseCaps;							// 人体识别能力
...
{
	BOOL							bSupportFaceRight;							// 是否支持目标左右角度过滤
	BOOL							bSupportFaceUp;								// 是否支持目标上下角度过滤
	BOOL							bSupportFaceRoll;							// 是否支持目标左右歪头角度过滤
...
///@brief  道路拥堵规则
typedef struct tagCFG_CONGESTION_SUPPORTED_RULES
{
	BOOL				bSupportLocalDataStore;							// 是否支持报表
	char				szReserved[1020];								// 预留字节
}CFG_CONGESTION_SUPPORTED_RULES;

///@brief  停车上限规则
typedef struct tagCFG_VEHICLE_LIMIT_SUPPORTED_RULES
{
	BOOL				bSupportLocalDataStore;							// 是否支持报表
	char				szReserved[1020];								// 预留字节
}CFG_VEHICLE_LIMIT_SUPPORTED_RULES;

///@brief  车辆态势规则
typedef struct tagCFG_VEHICLE_POSTURE_SUPPORTED_RULES
{
	BOOL				bSupportLocalDataStore;							// 是否支持报表
	BOOL				bSupportSensitivity;							// 是否支持灵敏度
	char				szSupportDetectTypeList[32][16];				// 支持的态势检测模式:"Percent",按百分比;"Vehicle",按车辆数
	int					nSupportDetectTypeListNum;						// 态势检测模式数量
	char				szReserved[1020];								// 预留字节
}CFG_VEHICLE_POSTURE_SUPPORTED_RULES;

///@brief  支持的规则
typedef struct tagCFG_VEHICLES_SUPPORTED_RULES
{	
	CFG_CONGESTION_SUPPORTED_RULES		stuCongestionDetection;								// 道路拥堵规则
	CFG_VEHICLE_LIMIT_SUPPORTED_RULES	stuVehicleLimitDetection;							// 停车上限规则
	CFG_VEHICLE_POSTURE_SUPPORTED_RULES	stuVehiclePostureDetection;							// 车辆态势规则
	char								szReserved[1024];									// 预留字节
}CFG_VEHICLES_SUPPORTED_RULES;

///@brief  标定线段相关能力
typedef struct tagCFG_VEHICLES_CALIBRATION
{
	UINT							nHorizontalLines;									// 水平线段数量
	UINT							nVerticalLines;										// 垂直线段数量
	char							szReserved[1024];									// 预留字节
}CFG_VEHICLES_CALIBRATION;

///@brief  车辆密度
typedef struct tagCFG_VEHICLES_DISTRI
{
	CFG_VEHICLES_SUPPORTED_RULES	stuSupportedRules;									// 支持的规则
	CFG_VEHICLES_CALIBRATION		stuCalibration;										// 标定线段相关能力
	UINT							nMaxRules;											// 支持的最大规则数
	char							szReserved[1020];									// 预留字节
}CFG_VEHICLES_DISTRI;

///@brief  人群分布图规则
typedef struct tagCFG_CROWD_DISTRI_MAP_RULE
{
	BOOL							bSupportPolygon;									// 人群分布图统计区是否支持任意边形
	BOOL							bSupportLocalDataStore;								// 是否支持报表
	char							szFeatureList[32][16];								// AI预览中支持展示的属性:"Event",事件;"Type",报警类型;"PeopleNum",人数;"Percent",百分比
	int								nFeatureListNum;									// 属性列表数量
	char							szReserved[1024];									// 预留字节
}CFG_CROWD_DISTRI_MAP_RULE;

///@brief  人群态势检测
typedef struct tagCFG_CROWD_POSTURE_DETECTION_RULE
{
	BOOL							bSupportSensitivity;								// 是否支持灵敏度
	BOOL							bSupportLocalDataStore;								// 是否支持报表
	UINT							nMiniNumLimit[2];									// 人群态势小模型下人数检测范围
	char							szFeatureList[32][16];								// AI预览中支持展示的属性:"Event",事件;"Type",报警类型;"PeopleNum",人数;"Percent",百分比
	char							szSupportDetectTypeList[32][16];					// 支持的态势检测模式:"Percent",按百分比;"Person",按人数
	int								nFeatureListNum;									// 属性列表数量
	int								nSupportDetectTypeListNum;							// 态势检测模式列表数量
	char							szReserved[1024];									// 预留字节
}CFG_CROWD_POSTURE_DETECTION_RULE;


///@brief  支持的规则
typedef struct tagCFG_CROWD_SUPPORTED_RULES
{
	CFG_CROWD_DISTRI_MAP_RULE			stuCrowdDistriMap;									// 人群分布图规则
	CFG_CROWD_POSTURE_DETECTION_RULE	stuCrowdPostureDetection;							// 人群态势检测	
	char								szReserved[1024];									// 预留字节
}CFG_CROWD_SUPPORTED_RULES;

///@brief  大模型能力
typedef struct tagCFG_CROWD_LLM_INFO
{
	UINT							nLargeNumLimit[2];									// 人数检测范围
	BOOL							bSupportUmbrella;									// 大模型下是否支持打伞检测
	BOOL							bSupportGlobalSensitivity;							// 大模型是否支持全局灵敏度
	BOOL							bSupportAiView;										// 是否支持大模型下AI预览
	BOOL							bSupport;											// 是否支持大模型
	BOOL							bSupportOSDDisplay;									// 是否支持码流显示大模型信息
	UINT							nEnableCtrl;										// 大模型的使能控制策略:0,不支持控制;1,支持控制;2,支持控制
	char							szModelList[32][64];								// 支持的大模型列表
	int								nModelListNum;										// 支持的大模型列表数量
	char							szReserved[1024];									// 预留字节
}CFG_CROWD_LLM_INFO;

///@brief  人群分布图
typedef struct tagCFG_CROWD_DISTRI_MAP
{
	CFG_CROWD_SUPPORTED_RULES		stuSupportedRules;									// 支持的规则
	CFG_VEHICLES_CALIBRATION		stuCalibration;										// 标定线段相关能力
	CFG_CROWD_LLM_INFO				stuLLM;												// 大模型能力
	UINT							nMaxRules;											// 支持的最大规则数
	char							szReserved[1020];									// 预留字节
}CFG_CROWD_DISTRI_MAP;
...
typedef struct tagCFG_CAP_SCENE
{
	...
	CFG_VEHICLES_DISTRI				*pstuVehiclesDistri;						// 车辆密度,由用户申请，内存大小为sizeof(CFG_VEHICLES_DISTRI)
	CFG_CROWD_DISTRI_MAP			*pstuCrowdDistriMap;						// 人群分布图,由用户申请，内存大小为sizeof(CFG_CROWD_DISTRI_MAP)
	BYTE              byReserved[1024 - 2*CFG_POINTERSIZE];                         // 保留字节
}CFG_CAP_SCENE;
...
{	
	...
	char				 szSupportedObjectTypes[32][32];						// 支持的检测物体类型
	int					 nSupportedObjectTypesNum;								// 支持的检测物体类型数量
	char				 szReserved[1020];										// 保留字节
...
typedef enum tagEM_VIDEO_ANALYSE_OBJECT_TYPE
{
    ...
    EM_VIDEO_ANALYSE_OBJECT_TYPE_HUMANFACE,         // 目标
...
typedef enum tagEM_VIDEO_ANALYSE_HUMANFACE_TYPE
{
    ...
    EM_VIDEO_ANALYSE_HUMANFACE_TYPE_NORMAL,         // 普通目标
    ...
    EM_VIDEO_ANALYSE_HUMANFACE_TYPE_ADJACENTFACE,   // 相邻目标 (注，ATM，尾随)
    EM_VIDEO_ANALYSE_HUMANFACE_TYPE_ABNORMALFACE,   // 异常目标
    EM_VIDEO_ANALYSE_HUMANFACE_TYPE_HELMETFACE,     // 头盔目标(如摩托车盔)
...
typedef struct
{
    ...
    int                                         nSupportFaceTypeNum;                                     // 支持的目标检测类型个数
    EM_VIDEO_ANALYSE_HUMANFACE_TYPE             emSupportFaceType[MAX_HUMANFACE_LIST_SIZE];              // 支持的目标检测类型列表
    BOOL                                        bTriggerPosition;                                        // 是否支持触发报警位置
    BOOL			                            bTriggerTrack;											 // 是否支持触发跟踪
	FACEBODY_DETECT_CAPS						stuFaceBodyDetectCaps;									 // 人体检测能力
	FACEBODY_ANALYSE_CAPS						stuFaceBodyAnalyseCaps;							         // 人体识别能力
...
typedef struct tagCFG_FACERECOGNITION_SCENCE_INFO
{
	...
	char                byFaceAngleDown;                        // 需要检测的目标向下最大偏角, 单位度，-45~45，负数表示目标向画面上边，正数表示目标向画面下边，0表示目标垂直方向上正对着摄像头。
	char                byFaceAngleUp;                          // 需要检测的目标向上最大偏角,单位度，-45~45，负数表示目标向画面上边，正数表示目标向画面下边，0表示目标垂直方向上正对着摄像头。
 	char                byFaceAngleLeft;                        // 需要检测的目标向左最大偏角,单位度，-45~45，负数表示目标向画面左边，正数表示目标向画面右边，0表示目标水平方向上正对着摄像头
	char                byFaceAngleRight;                       // 需要检测的目标向右最大偏角,单位度，-45~45，负数表示目标向画面左边，正数表示目标向画面右边，0表示目标水平方向上正对着摄像头
	EM_FACEDETECTION_TYPE	emDetectType;						// 目标检测类型
...
typedef struct tagCFG_PARKING_SPACE
{
	...
	char			szAssociateDevice[16][32];					/// 关联到的外设唯一标识
	int				nAssociateDeviceNum;						/// 关联到的外设唯一标识个数
	char			szReserved[1020];							/// 预留字节
}CFG_PARKING_SPACE;
...
{
	UINT						nFaceAngleUp;								// 需要检测的目标向上(向下)最大偏角,超过此角度不上报,单位度,0-90。
	UINT						nFaceAngleRight;							// 需要检测的目标向右(向左)最大偏角,超过此角度不上报,单位度,0-90。
	UINT						nFaceRollRight;								// 需要检测的目标向右（向左）歪头最大偏角,超过此角度不上报,单位度,0-90。
	BOOL                        bTempOptimization;                          // 是否开启智能优选
    BOOL                        bEyesWidthDetection;                        // 是否开启检测，换算出目标距离
...
{
	CFG_EM_FACEDETECT_TYPE			emFaceDetectType;								// 目标检测智能类型
...
typedef struct tagCFG_ANALYSEGLOBAL_SCENE
{
	...
	union
	{
		CFG_FACEDETECTION_SCENCE_INFO	stuFaceDetectionScene;	// 目标检测场景/目标识别检查
...
typedef struct tagCFG_ANALYSEGLOBAL_INFO
{
	...
	BOOL				            bFaceDetection;							// 目标检测场景是否有效
	CFG_FACEDETECTION_SCENCE_INFO   stuFaceDetectionScene;		            // 目标检测场景
	CFG_TIME_PERIOD		            stuDayTimePeriod;						// 标定白天的时间段.(8,20),表示从8点到晚上20点为白天
	CFG_TIME_PERIOD		            stuNightTimePeriod;						// 标定黑夜的时间段.(20,7)，表示从晚8点到凌晨7点为黑夜
...
///@brief 坐标点
typedef struct tagNET_CFG_UINT_POINT
{
	UINT	nX;
	UINT	nY;
} NET_CFG_UINT_POINT;
...
///@brief 人Lian美化
typedef struct tagCFG_FACE_BEAUTIFICATION
{
	BOOL	bEnable;				/// 使能
	UINT		nBeautyLevel;		/// 降噪等级取值为0~100，默认值50, 等级0与人Lian美化使能不开效果一致，等级越高，降噪程度越大，对人Lian美化效果越高
	UINT		nModelType;			/// 0:手动，1：自动，默认手动
	UINT		nEeLevel;			/// 人Lian边缘锐度参数[0, 100]，默认50
	UINT		nBlackEdge;			/// 黑边抑制，取值范围[0, 100] 默认 50，值越大，黑边越明显
	UINT		nWhiteEdge;			/// 白边抑制，取值范围[0, 100],默认 50 ，值越大，白边越明显
	UINT		nBrightness;			/// 亮度等级，取值[0,100], 默认50，值越大，亮度越高
	UINT		nSaturation;			/// 颜色等级，取值[0,100], 默认50，值越大，颜色越鲜艳
	char							szReserved[128];							// 保留字节
} CFG_FACE_BEAUTIFICATION;

///@brief 车道信息扩展
typedef struct tagCFG_LANE_EX
{
	int				nRoadwayDirection;			///车道方向 0:未知 1: 南向北 2: 西南向东北 3: 西向东 4: 西北向东南 5: 北向南 6: 东北向西南 7: 东向西 8: 东南向西北 9: 忽略 10: 自定义
	char				szReserved[60];							// 保留字节
}CFG_LANE_EX;

///@brief 视频分析全局配置
typedef struct tagCFG_ANALYSEGLOBAL_EX_INFO
{
	char				            szSceneType[MAX_NAME_LEN];				// 应用场景,详见"支持的场景列表"
	...
	int                  			nParkingSpaceNumEx;                     // 车位数，为nParkingSpaceNum的扩展字段，后续使用此字段                         
	CFG_PARKING_SPACE    			stParkingSpacesEx[64];					// 车位配置信息,每个元素代表一个车位，为stParkingSpaces的扩展字段，后续使用此字段

	int                             nLightGroupNumEx;                         // 灯组数 为nLightGroupNum的扩展字段，后续使用此字段
	CFG_LIGHTGROUPS_V2              stLightGroupsEx[8];      // 灯组配置信息, 为stLightGroups的扩展字段，后续使用此字段
	CFG_POLYLINE					stuPositionMatchLine[20];				// 外设抓拍物体匹配线
	int								nPositionMatchLineNum;					// 外设抓拍物体匹配线个数
	UINT							nPositionDetectRegionNum;				// 车辆位置检测区域个数
	NET_CFG_UINT_POINT				stuPositionDetectRegion[20];			// 车辆位置检测区域
	CFG_FACE_BEAUTIFICATION	 stuFaceBeautification;	/// 人Lian美化
	int							nOSDMode;							// 客流OSD叠加方式，默认智能帧解析。 0：智能帧解析； 1：视频叠加
	int                             nLaneNumEx;                               // 车道数
	CFG_LANE_EX                        stuLanesEx[8];                 // 车道信息
	NET_CFG_UINT_POINT				stuStraightDivisionLine[20];			// 直行分界线
	UINT							nStraightDivisionLineNum;				// 直行分界线顶点有效个数
	UINT							nFaceAngleUp;							/// 需要检测的人脸向上最大偏角,超过此角度不上报,单位度，0-45，正数表示人脸向画面下边
	UINT							nFaceAngleRight;						/// 需要检测的人脸向右最大偏角,超过此角度不上报,单位度，0~45，正数表示人脸向画面右边
	char							szResvered[4];							// 保留字节
} CFG_ANALYSEGLOBAL_EX_INFO;
...
typedef struct tagCFG_SIZEFILTER_INFO
{
	...
	bool				bCfgSizeUseUInt;						  // 兼容字段,置为true则CFG_SIZE结构体内部使用UINT类型下发
...
typedef struct tagCFG_MODULE_INFO
{
	...
	bool                bShadow;                                 // 是否有阴影
	bool                bContourAssistantTrack;                  // 是否开启轮廓辅助跟踪，例：在目标识别时可以通过跟踪人体来辅助识别目标
	...
	int					nDetectLineMode;						//辅助线模式，开关门检测智能大类使用        0:未知 1:中线 2:边线
	int                 nDetectLineNum;							//警戒线数量
	CFG_POLYGON 		stuDetectLine[20];			//警戒线折线类型，折线中每个端点的坐标归一化到[0,8192)区间。
	BOOL 				bMotorcycleAsMotor;						/// 摩托车是否作为机动车处理
...
///@brief 绝对时间信息
typedef struct tagNET_ABSOLUTE_TIME_INFO
{
    NET_TIME                stuStartTime;       /// 开始时间
    NET_TIME                stuStopTime;        /// 结束时间
    BOOL                    bEnable;            /// 绝对时间使能
    char                    szReserved[28];     /// 预留字节
}NET_ABSOLUTE_TIME_INFO;
...
typedef struct tagCFG_CROSSLINE_INFO
{
    ...
    NET_ABSOLUTE_TIME_INFO  stuAbsoluteTime[24];                                    /// 绝对时间数组
    UINT                    nAbsoluteTimeNum;                                       /// 绝对时间数组信息有效个数
	char					szReserved[1020];										/// 预留字节
} CFG_CROSSLINE_INFO;
...
///@brief 区域顶点信息
typedef struct tagNET_CFG_POLYGON
{
	UINT			nX; 				// X坐标
	UINT			nY;	 				// Y坐标
	char			szReserved[32];		// 预留字节
}NET_CFG_POLYGON;

///@brief 排除区域信息
typedef struct tagNET_EXCLUDE_REGION_INFO 
{
	NET_CFG_POLYGON		stuExcludeRegion[20];	// 排除区域
	int					nDetectRegionPoint;		// 排除区域有效顶点数
	char				szReserved[60];			// 预留字节
}NET_EXCLUDE_REGION_INFO;

///@brief 主动巡视类型
typedef enum tagEM_PATROL_TYPE
{
	EM_PATROL_TYPE_UNKNOWN,				// 未知
	EM_PATROL_TYPE_PRESET,				// 预置点主动巡航
}EM_PATROL_TYPE;

///@brief 预置点配置列表信息
typedef struct tagNET_PRESET_CONFIG_LIST_INFO 
{
	UINT                nIndex;								// 预置点编号，从1开始编号
	UINT 				nStayTime;							// 预置点到位后的停留时间(单位：秒)，取值范围1-3600
	char				szReserved[128];					// 预留字节
}NET_PRESET_CONFIG_LIST_INFO;

///@brief 预置点主动巡视具体信息
typedef struct tagNET_PRESET_INFO 
{
	NET_PRESET_CONFIG_LIST_INFO*	pstList;							// 预置点配置列表，内存由用户申请释放
	UINT 							nMaxListNum;						// 预置点配置内存申请个数
	UINT 							nRetListNum;						// 预置点配置有效返回个数，获取配置时作为出参
	UINT							nArriveTimeout;						// 预置点到位超时时间(单位：秒)，取值范围5-300
	char							szReserved[252 - CFG_POINTERSIZE];	// 预留字节
}NET_PRESET_INFO;

///@brief 周界主动巡视信息
typedef struct tagNET_PARTOL_LINKAGE_INFO
{
	BOOL                bEnable;							// 是否开启主动巡视
	EM_PATROL_TYPE		emPatrolType;						// 主动巡视类型
	NET_PRESET_INFO		stuPreset;							// 预置点主动巡视具体信息
	char				szReserved[1024];					// 预留字节
}NET_PARTOL_LINKAGE_INFO;
...
typedef struct tagCFG_RIOTER_INFO 
{
	...
	UINT				nDetectModesNum;										// 当前规则的检测模式有效个数
	UINT				nDetectModes[64];										// 当前规则的检测模式
	char				szCompareMode[32];										// 人数聚集的检测方式，对RioterThreshold参数的判断方式
																				// "COMPARE_MODE_MORE" 大于, "COMPARE_MODE_LESS" 小于 
																				// "COMPARE_MODE_NOT_EQUAL" 不等于, "COMPARE_MODE_EQUAL" 等于, "COMPARE_MODE_NONE" 不做比较,直接输出人数
	UINT						nCompositeMode;									// 比较模式间的组合方式, 0 组合模式不生效, 1 采用"与"的方式组合, 2 采用"或"的方式组合
    UINT                		nRioterSecondThreshold;							// 组合模式时, 聚集人数阈值, CompositeMode不为0时生效
	char						szSecondCompareMode[32];						// 人数聚集的检测方式, 值同szCompareMode

	NET_EXCLUDE_REGION_INFO		stuExcludeRegions[20];							// 对应1个或多个排除区域
	NET_PARTOL_LINKAGE_INFO		stuPatrolLinkage;								// 主动巡视配置信息
	UINT                        nExcludeRegionsNum;								// 排除区域有效个数
	char						szReserved[1020];								// 预留字节
} CFG_RIOTER_INFO;

///@brief 计量方式
typedef enum tagEM_MEASUER_MODE
{
	EM_MEASUER_MODE_UNKNOWN,				// 未知
	EM_MEASUER_MODE_PIXEL,					// 像素 
	EM_MEASUER_MODE_METRIC,					// 实际长度，单位：米
	EM_MEASUER_MODE_CALIBRATED_PIXEL,		// 远端近端标定后的像素 
}EM_MEASUER_MODE;

///@brief 过滤类型
typedef enum tagEM_FILTER_TYPE
{
	EM_FILTER_TYPE_UNKNOWN,					// 未知
	EM_FILTER_TYPE_BY_LENGTH,				// 按照宽度或高度 
	EM_FILTER_TYPE_BY_WIDTH_HEIGHT,			// 按照宽度和高度
	EM_FILTER_TYPE_BY_WIDTH,				// 按照宽度
}EM_FILTER_TYPE;

///@brief 尺寸
typedef struct tagNET_OBJECT_SIZE_INFO
{
	UINT					nWidth;				// 宽度
	UINT					nHeight;			// 高度
	char					szReserved[32];		// 预留字节
}NET_OBJECT_SIZE_INFO;

///@brief 长宽校准框信息
typedef struct tagNET_CALIBRATE_BOXS_INFO
{
	NET_CFG_POLYGON			stuCenterPoint;		// 校准框中心点坐标, [0, 8192)
	double					dbRatio;			// 对基准校准框的比率
	char					szReserved[252];	// 预留字节
}NET_CALIBRATE_BOXS_INFO;

///@brief 面积校准框信息
typedef struct tagNET_AREA_CALIBRATE_BOXS_INFO
{
	NET_CFG_POLYGON			stuCenterPoint;		// 校准框中心点坐标, [0, 8192)
	double					dbRatio;			// 对基准校准框的比率
	char					szReserved[252];	// 预留字节
}NET_AREA_CALIBRATE_BOXS_INFO;

///@brief 比率校准框信息
typedef struct tagNET_RATIO_CALIBRATE_BOXS_INFO
{
	NET_CFG_POLYGON			stuCenterPoint;		// 校准框中心点坐标, [0, 8192)
	double					dbRatio;			// 对基准校准框的比率
	char					szReserved[252];	// 预留字节
}NET_RATIO_CALIBRATE_BOXS_INFO;

///@brief 尺寸过滤器
typedef struct tagNET_SIZEFILTER_INFO
{
	EM_MEASUER_MODE					emMeasureMode;				// 计量方式
	BOOL							bBySize;					// 简单过滤，是否按长宽过滤
	BOOL							bByArea;					// 复杂过滤，是否按面积过滤
	EM_FILTER_TYPE					emType;						// 过滤类型
	NET_OBJECT_SIZE_INFO			stuMinSize;					// 物体最小尺寸
	NET_OBJECT_SIZE_INFO			stuMaxSize;					// 物体最大尺寸
	double							dbMinArea;					// 最小面积
	double							dbMaxArea;					// 最大面积
	NET_OBJECT_SIZE_INFO			stuMinAreaSize;				// 最小面积矩形框尺寸
	NET_OBJECT_SIZE_INFO			stuMaxAreaSize;				// 最大面积矩形框尺寸
	double							dbMinRatio;					// 最小宽高比, 取值范围0.001~1000
	double							dbMaxRatio;					// 最大宽高比, 取值范围0.001~1000
	NET_OBJECT_SIZE_INFO			stuMinRatioSize;			// 最小宽高比矩形框尺寸
	NET_OBJECT_SIZE_INFO			stuMaxRatioSize;			// 最大宽高比矩形框尺寸
	BOOL							bByRatio;					// 复杂过滤，是否按宽高比过滤
	UINT							nCalibrateBoxsNum;			// 长宽校准框有效个数
	NET_CALIBRATE_BOXS_INFO			stuCalibrateBoxs[10];		// 长宽校准框
	NET_AREA_CALIBRATE_BOXS_INFO	stuAreaCalibrateBoxs[10];	// 面积校准框
	UINT							nAreaCalibrateBoxsNum;		// 面积校准框有效个数
	UINT							nRatioCalibrateBoxsNum;		// 比率校准框有效个数
	NET_RATIO_CALIBRATE_BOXS_INFO	stuRatioCalibrateBoxs[10];	// 比率校准框
	NET_OBJECT_SIZE_INFO			stuNearMinSize;				// 近景物体最小尺寸
	NET_OBJECT_SIZE_INFO			stuNearMaxSize;				// 近景物体最大尺寸
	NET_OBJECT_SIZE_INFO			stuFarMinSize;				// 远景物体最小尺寸
	NET_OBJECT_SIZE_INFO			stuFarMaxSize;				// 远景物体最大尺寸
	char							szReserved[1024];			// 预留字节
}NET_SIZEFILTER_INFO;

///@brief 过滤目标类型 
typedef enum tagEM_CFG_FILTER_OBJECT
{
	EM_CFG_FILTER_OBJECT_UNKNOWN,				// 未知
	EM_CFG_FILTER_OBJECT_HUMAN,					// 人 
	EM_CFG_FILTER_OBJECT_VEHICLE,				// 机动车，不包括非机动车 
	EM_CFG_FILTER_OBJECT_NONMOTOR,				// 非机动车 
}EM_CFG_FILTER_OBJECT;

///@brief 事件类型EVENT_IVS_OBJECT_APPEAR_DETECTION(目标出现事件)对应的规则配置
typedef struct tagCFG_OBJECT_APPEAR_DETECTION_INFO
{
	...
	UINT							nMinDuration;								// 最短持续时间, 范围[1-3600], 单位：秒
	UINT							nReportInterval;							// 报警时间间隔, 取值范围[0-3600], 单位：秒
	UINT							nDetectMode;								// 检测模式: 0:未知, 1:自定义, 2:自适应
	int								nSensitivity;						   		// 灵敏度，取值[1-10]
	UINT						    nDetectRegionNum;							// 检测区顶点数
	NET_CFG_POLYGON				    stuDetectRegion[20];						// 检测区
	EM_CFG_FILTER_OBJECT			emFilterObjectList[16];						// 过滤目标
	UINT							nFilterObjectListCount;						// 过滤目标个数
	BOOL							bMovingTargetFilterEnable;		    		// 是否开启运动目标过滤
	NET_SIZEFILTER_INFO 	    	stuSizeFilter;                      		// 规则特定的尺寸过滤器
    BOOL                		    bSizeFilter;                        		// 规则特定的尺寸过滤器是否有效
	UINT							nAlgId;										// 规则使用的变化事件算法id
	char							szAlgName[128];								// 算法名称
	UINT							nAlarmId;									// 自定义报警ID
	char							szReserved[4092];							// 预留字节
}CFG_OBJECT_APPEAR_DETECTION_INFO;

///@brief 事件类型EVENT_IVS_OBJECT_DISAPPEAR_DETECTION(目标消失事件)对应的规则配置
typedef struct tagCFG_OBJECT_DISAPPEAR_DETECTION_INFO
{
	char							szRuleName[128];							// 规则名称,不同规则不能重名
	BOOL                    		bRuleEnable;								// 规则使能
	int                     		nObjectTypeNum;								// 相应物体类型个数
	...
	UINT							nAlarmId;									// 自定义报警ID
	char							szReserved[4092];							// 预留字节
}CFG_OBJECT_DISAPPEAR_DETECTION_INFO;

///@brief 联动注册库信息
typedef struct tagNET_CFG_STATE_DETECTION_LINKGROUP_INFO
{
	char                            szGroupID[64];                          	// 当前视频通道对应的布控组ID
    char                            szGroupName[20];                        	// 注册库名称
    BOOL                            bEnable;                                	// 注册库是否启用
    int                            	nSimilarity;                            	// 相似度阈值
    char                            szReserved[1020];                       	// 预留字节
}NET_CFG_STATE_DETECTION_LINKGROUP_INFO;

///@brief 事件类型EVENT_IVS_OBJECT_STATE_DETECTION(目标状态事件)对应的规则配置
typedef struct tagCFG_OBJECT_STATE_DETECTION_INFO
{
	char							szRuleName[128];							// 规则名称,不同规则不能重名
	BOOL                    		bRuleEnable;								// 规则使能
	int                     		nObjectTypeNum;								// 相应物体类型个数
	...
	UINT							    	nMinDuration;						// 最短持续时间, 范围[1-3600], 单位：秒
	UINT							    	nReportInterval;					// 报警时间间隔，取值范围[0-3600], 单位：秒
	int								    	nSensitivity;						// 灵敏度，取值1-10，数值越大代表灵敏度越高
	UINT							    	nDetectMode;						// 检测模式: 0:未知, 1:自定义, 2:自适应
	UINT						        	nDetectRegionNum;					// 检测区顶点数
	NET_CFG_POLYGON				        	stuDetectRegion[20];				// 检测区
	NET_SIZEFILTER_INFO 	        		stuSizeFilter;                      // 规则特定的尺寸过滤器
	...
	char							    	szAlgName[128];						// 算法名称
	UINT							    	nAlarmId;							// 自定义报警ID
    UINT                                	nLinkGroupNum;                     	// 联动注册库有效个数
    NET_CFG_STATE_DETECTION_LINKGROUP_INFO  stuLinkGroup[64];                   // 联动注册库
	char									szReserved[4096];					// 预留字节
}CFG_OBJECT_STATE_DETECTION_INFO;
...
///@brief 人数去重策略配置
typedef struct tagCFG_DUPLICATE_REMOVE_INFO
{
	BOOL				bEnable;										// 使能开关
	UINT                nSensitivity;                                // 人数去重灵敏度，取值1-10，默认5，值越小越不容易判断重复，值越高越容易被判断为重复，仅在支持人数去重时生效。
	int					nType;											// 策略类型 0: 按时刻表
	BOOL				bUseBeginTime;								/// 是否使用去重开始结束时间
	CFG_TIME_EX			stuBeginTime;								/// 去重起始时间, 仅时分有效
	CFG_TIME_EX			stuEndTime;									/// 去重结束时间, 仅时分有效
	char				szReserved[96];                        		// 保留字段
} CFG_DUPLICATE_REMOVE_INFO;
...
typedef struct tagCFG_NUMBERSTAT_INFO
{
    ...
    CFG_ALARM_MSG_HANDLE    stuRemoteEventHandler;                                        // 前端视频分析联动
    CFG_TIME_SECTION        stuRemoteTimeSection[WEEK_DAY_NUM][MAX_REC_TSECT_EX];         // 前端视频分析联动响应时间段
	CFG_DUPLICATE_REMOVE_INFO	stuDuplicateRemove;								//人数去重策略配置
	BOOL					bAttrEnable;							/// 人员目标属性使能开关，true:开启，false:关闭
	char					szReserved[1020];                        // 保留字段
} CFG_NUMBERSTAT_INFO;
...
typedef struct tagCFG_FACERECOGNITION_INFO
{
	...
	BYTE                 byMode;                                                // 对比模式, 0-正常, 1-指定目标区域组合, 2-智能模式(算法根据目标各个区域情况自动选取组合)
	BYTE                 byImportantRank;                                       // 查询重要等级大于等于此等级的人员(1~10,数值越高越重要)
	int                  nAreaNum;                                              // 区域数  
	BYTE                 byAreas[8];                                            // 目标区域组合, 0-眉毛，1-眼睛，2-鼻子，3-嘴巴，4-脸颊(此参数在对比模式为1时有效)
...
typedef struct tagCFG_TRAFFICTOLLGATE_INFO
{
	char				szRuleName[MAX_NAME_LEN];								// 规则名称,不同规则不能重名
	bool				bRuleEnable;											// 规则使能
	BYTE                nDirectionNum;											// 车辆行驶方向个数
	BYTE				nDirection[2];											// 相对车道方向需要上报的车辆行驶方向, 0:未知, 1:正向, 2:反向
	int					nObjectTypeNum;											// 相应物体类型个数
	char				szObjectTypes[MAX_OBJECT_LIST_SIZE][MAX_NAME_LEN];		// 相应物体类型列表
	int                 nPtzPresetId;											// 云台预置点编号	0~65535
...
///@brief 事件类型EVENT_IVS_FACEDETECT(目标检测事件)对应的规则配置
typedef struct tagCFG_FACEDETECT_INFO
{
	...
	int					nDetectRegionPoint;										// 检测区顶点数
	CFG_POLYGON			stuDetectRegion[MAX_POLYGON_NUM];						// 检测区
	int                 nHumanFaceTypeCount;                                    // 触发事件的目标类型个数
	char                szHumanFaceType[MAX_HUMANFACE_LIST_SIZE][MAX_NAME_LEN]; // 触发事件的目标类型
	CFG_ALARM_MSG_HANDLE stuEventHandler;										// 报警联动
	CFG_TIME_SECTION	stuTimeSection[WEEK_DAY_NUM][MAX_REC_TSECT_EX];			// 事件响应时间段
	int                 nPtzPresetId;											// 云台预置点编号	0~65535
    ...
    BOOL                bSizeFileter;                                           // 规则特定的尺寸过滤器是否有效
	CFG_SIZEFILTER_INFO stuSizeFileter;                                         // 规则特定的尺寸过滤器
	BOOL				bFeatureEnable;											// 是否开启目标属性识别, IPC增加
																				//(通过FaceDetection能力中的FeatureSupport来确定该配置是否可以设置)
	int					nFaceFeatureNum;										// 需要检测的目标属性个数
	EM_FACEFEATURE_TYPE emFaceFeatureType[MAX_FEATURE_LIST_SIZE];				// 需检测的目标属性, 通过FaceDetection能力来获取支持哪些目标属性
	CFG_FACE_BEAUTIFICATION	 stuFaceBeautification;	/// 人Lian美化
...
typedef struct tagCFG_LINKGROUP_INFO
{
	...
	char					szColorName[MAX_COLOR_NAME_LEN];	// 事件触发时绘制目标框的颜色
...
{
	BOOL 					bEnable;							// 模式是否启用
	char					szColorHex[MAX_COLOR_HEX_LEN];		// 事件触发时绘制目标框的颜色
	BOOL 					bShowTitle;							// 事件触发时规则框上是否显示报警标题
	BOOL 					bShowPlate;							// 事件触发时是否显示比对面板
...
///@brief 事件类型EVENT_IVS_FACEANALYSIS(目标分析事件) 对应的规则配置
typedef struct tagCFG_FACEANALYSIS_INFO
{
	...
	BOOL					bFeatureEnable;											// 是否开启目标属性识别, IPC增加
	int						nFaceFeatureNum;										// 需要检测的目标属性个数
	EM_FACEFEATURE_TYPE 	emFaceFeatureType[MAX_FEATURE_LIST_SIZE];				// 需检测的目标属性
	BOOL					bFeatureFilter;											// 在目标属性开启前提下，如果目标图像质量太差，是否不上报属性
																					// true-图像太差不上报属性 false-图像很差也上报属性(可能会非常不准，影响用户体验)
	int						nMinQuality;											// 目标图片质量阈值,和bFeatureFilter一起使用 范围[1,100]
	CFG_FACE_BEAUTIFICATION	 stuFaceBeautification;	/// 人Lian美化
	char                szReserved[1024];                                       // 保留字段
} CFG_FACEANALYSIS_INFO;

///@brief 事件类型EVENT_IVSS_FACEATTRIBUTE(IVSS目标检测事件) 对应的规则配置
typedef struct tagCFG_FACEATTRIBUTE_INFO
{
	...
	int                 nPtzPresetId;											// 云台预置点编号	0~65535
	int                 nMinDuration;                                           // 最短触发时间,单位：秒 
	int                 nTriggerTargetsNumber;                                  // 触发报警的目标个数
	...
	EM_FACEFEATURE_TYPE emFaceFeatureType[MAX_FEATURE_LIST_SIZE];				// 需检测的目标属性
	BOOL                bFeatureFilter;                                         // 在目标属性开启前提下，如果目标图像质量太差，是否不上报属性
	int                 nMinQuality;                                            // 目标图片质量阈值,和bFeatureFilter一起使用 范围[0,100]
...
///@brief 事件类型EVENT_IVS_TRAFFIC_PARKING_SPACEDETECTION(违停相机单球车位检测)规则配置
...
///@brief 事件类型 EVENT_IVS_TRAFFIC_WRONGROUTE (不按车道行驶事件)对应的规则配置
///@brief 事件类型 EVENT_IVS_TRAFFIC_CROSSLANE (变道)对应的规则配置
///@brief 事件类型 EVENT_IVS_TRAFFIC_OVERLINE (压车道线)对应的规则配置
///@brief 事件类型 EVENT_IVS_TRAFFIC_TURNLEFT (左转事件)对应的规则配置
///@brief 事件类型 EVENT_IVS_TRAFFIC_TURNRIGHT (右转)对应的规则配置

///@brief 事件类型 EVENT_IVS_TRAFFIC_YELLOWPLATEINLANE (黄牌车占道事件)对应的规则配置
///@brief 事件类型 EVENT_IVS_TRAFFIC_VEHICLEINROUTE (有车占道事件)对应的规则配置
///@brief 事件类型 EVENT_IVS_TRAFFIC_BACKING (倒车事件)对应的规则配置
...
///@brief 事件类型 EVENT_IVS_TRAFFIC_OVERYELLOWLINE (压黄线)对应的规则配置
...
///@brief 事件类型 EVENT_IVS_TRAFFIC_RETROGRADE (逆行事件)对应的规则配置
...
///@brief 事件类型 EVENT_IVS_TRAFFIC_UTURN (掉头)对应的规则配置
...
typedef struct tagCFG_TRAFFIC_OVERSPEED_INFO
{
	...
	int					nBigCarUnderSpeedMargin;								// 大车限低速宽限值
	int 				nBigCarOverSpeedMargin;									// 大车限高速宽限值
	int 				nUnderSpeedMargin;										// 小车限低速宽限值
	int 				nOverSpeedMargin;										// 小车限高速宽限值
	int					nBigCarSpeedUpperLimit;									// 大车车速度上限
	int					nBigCarSpeedLowerLimit;									// 大车车速度下限
	int					nYellowSpeedUpperLimit;									// 黄牌车限速上限
	int 				nYellowSpeedLowerLimit;									// 黄牌车限速下限
	bool				bSnapMotorcycle;										// 是否抓拍摩托车
	BYTE				bSnapNoPlateMotor;										// 是否抓拍无牌机动车,0为不抓拍无牌机动车,1为抓拍无牌机动车,默认为0
	BYTE                bReserved2[2];                                          // 保留字段
}CFG_TRAFFIC_OVERSPEED_INFO;

///@brief 事件类型 EVENT_IVS_TRAFFIC_UNDERSPEED (欠速)对应的规则配置
...
typedef struct tagCFG_TRAFFIC_PEDESTRAINPRIORITY_INFO
{
	...
	int					nSnapPicHavePriCategory;								// 抓拍的图片是否要有行人或非机动车的掩码值每个bit位含义，1：抓拍图片要有行人；0：抓拍图片可以无行人
																				// bit0:第一张图片是否需要有行人
																				// bit1:第二张图片是否需要有行人
																				// bit2:第三张图片是否需要有行人
...
typedef struct tagCFG_TRAFFIC_PARKINGONYELLOWBOX_INFO
{
	...
	int					nFollowTime;											// 跟随时间，如果一辆车与前一辆车进入黄网格的时间差小于此值，就认为是跟车进入
...
typedef struct tagCFG_ELEVATOR_ABNORMAL_INFO
{
	...
	int						nAbnormalActionNum;							//	检测扶梯异常动作数量
	int						nAbnormalAction[2];								// 检测扶梯异常动作: 0:未知 1: 扶梯骤停 2: 扶梯逆转
	BYTE                bReserved[500]; 										// 保留字节
...
typedef struct tagCFG_HUMANTRAIT_INFO
{
    ...
    int                             nHumanFaceTypeNum;                                  // 支持的目标检测类型个数
    EM_VIDEO_ANALYSE_HUMANFACE_TYPE emHumanFaceType[MAX_HUMANFACE_LIST_SIZE];           // 支持的目标检测类型列表

    int                         nMinDuration;                                           // 最短触发时间,单位：秒
    int                         nTriggerTargets;                                        // 触发报警的目标个数
    int                         nSensitivity;                                           // 灵敏度,越高越容易检测, 0-10
    BOOL                        bSizeFileter;                                           // 是否使用尺寸过滤器
    CFG_SIZEFILTER_INFO         stuSizeFileter;                                         // 规则特定的尺寸过滤器，提高判断精度
    BOOL                        bFeatureEnable;                                         // 是否开启目标属性识别
    BOOL                        bFeatureFilter;                                         // 在目标属性开启前提下，如果目标图像质量太差，是否不上报属性  true-图像太差不上报属性;false-图像很差也上报属性(可能会非常不准，影响用户体验)
    int                         nMinQuality;                                            // 目标图片质量阈值,和bFeatureFilter一起使用
...
typedef struct tagCFG_PHONECALL_DETECT_INFO
{
	...
	CFG_SIZEFILTER_INFO		stuSizeFilter;										// 规则特定的尺寸过滤器，为提高规则判断精度
	BOOL				bRelateFace;											// 关联目标, 抓拍目标使能开关，false:关闭，true：开启，默认关闭
	BYTE				byReserved[3608]; 										// 保留字节
...
{   
	...
	BOOL							bHighEnable;								// 温度异常报警是否开启
	BOOL							bLowEnable;									// 温度异常报警是否开启
	int								fHighThresholdTemp;							// 温度异常阈值，精度0.1，扩大10倍
	int								fLowThresholdTemp;							// 温度异常阈值，精度0.1，扩大10倍
	BOOL							bIsAutoStudy;								// 是否自动学习
	int								fHighAutoOffset;							// 温度自动学习偏差值，精度0.1，扩大10倍
	int								fLowAutoOffset;								// 温度自动学习偏差值，精度0.1，扩大10倍
	int								nSensitivity;								// 灵敏度 范围[1, 10]
	BOOL							bSizeFileter;								// 规则特定的尺寸过滤器是否有效
	CFG_SIZEFILTER_INFO				stuSizeFileter;								// 规则特定的尺寸过滤器
...
typedef struct tagCFG_PORTRAIT_FACE_INFO
{
    ...
    UINT                    nSensitivity;                                           // 目标抓拍灵敏度, 数字越大越灵敏, 范围[0,100]
    UINT                    nMinQuality;                                            // 目标质量阈值, 范围[1,100]
    BOOL                    bFeatureEnable;                                         // 是否开启目标属性识别
    CFG_SIZEFILTER_INFO     stuSizeFileter;                                         // 规则特定的尺寸过滤器
    BOOL                    bSizeFileter;                                           // 规则特定的尺寸过滤器是否有效
    int                     nFeatureListNum;                                        // 目标属性个数
    EM_FACEFEATURE_TYPE     emFeatureList[32];                                      // 目标属性列表, bFeatureEnable为true有效
    BOOL                    bFeatureFilter;                                         // 在目标属性开启前提下，如果目标图像质量太差，是否不上报属性
    BOOL                    bFaceEnable;                                            // 是否开启目标检测
    BYTE                    byReserved[256];                                        // 预留字节
...
typedef struct tagCFG_PORTRAIT_DETECTION_INFO
{
    ...
    CFG_PORTRAIT_FACE_INFO  stuFaceInfo;                                           // 目标规则
    ...
    UINT                    nSnapThreshold;                                         // 目标抓拍过滤阈值，数值越高过滤强度越大, 范围[1,100]
...
typedef enum tagEM_CFG_CONVEYER_BELT_DETECT_TYPE
{
	...
	EM_CFG_CONVEYER_BELT_DETECT_MOVETOSTOP,			// 运动到静止 
	EM_CFG_CONVEYER_BELT_DETECT_STOPTOMOVE,			// 静止到运动 
	EM_CFG_CONVEYER_BELT_DETECT_LOADINGRATE,		// 装载率
...
///@brief 报警等级阈值
typedef struct tagCFG_ALARM_LEVEL_THRESHOLD
{
	int						nMinThreshold;			// 小阈值
	int						nMaxThreshold;		// 大阈值
	int						nMinCmpType;		    // 小阈值的比较模式，0：未知；1：不比较；2：大于；3：大于等于；4：小于；5：小于等于
	int						nMaxCmpType;			// 大阈值的比较模式，0：未知；1：不比较；2：大于；3：大于等于；4：小于；5：小于等于
	int						nAlarmLevel;				// 报警等级，范围1~5
	int						nAlarmRuleType;		// 0:未知 1:"NonLoad"空载 2:"RunOff"跑偏 3:"Block"阻塞 4:"Bulk"大块异物 
	char					    szReserved[64]; 		// 保留字节
}CFG_ALARM_LEVEL_THRESHOLD;
...
typedef struct tagCFG_CONVEYER_BELT_DETECT_INFO
{
	...
	CFG_ALARM_LEVEL_THRESHOLD		stuAlarmLevelThreshold[20];		//报警等级阈值，最多20组，每组包含一组阈值和一个报警等级
	//跑偏：可设置多组阈值，范围0~100
	//空载：设置一组阈值，范围为0~15
	//大块物：煤炭矿业大块物检测可设置多组报警等级阈值，范围[1-200]，其他可不用配置等级阈值
	//阻塞：设置一组阈值，范围0~100
	int					nAlarmLevelThresholdNum;							// 报警等级阈值组数
	UINT					nLoadingRateReportInterval;							// 装载率上报间隔 单位：秒 范围[1-1800]
	UINT					nLoadingThreshold;										// 装载率报警阈值;取值0-10
	BYTE                    byReserved[2264];                                       // 保留字节
...
{
	int						nSurfaceNumber;											// 区域绑定加油机面号
	int						nNodeNumber;											// 区域绑定加油机节点号
	char					szOilType[8];											// 对应油号
...
typedef struct tagREGIONS_INFO
{
	...
	int						nGasPumpNozzleInfoNum;									// 加油机信息个数
	char					szReserved3[4];											// 字节对齐
	GAS_PUMP_NOZZLE_INFO	stuGasPumpNozzleInfo[8];								// 加油机信息
	char                    szReserved[1024];                                       // 保留字节
}REGIONS_INFO;
...
///@brief 姿态类型
typedef enum tagEM_ATTITUDE_TYPE
{
	EM_ATTITUDE_TYPE_UNKNOWN,					// 未知
	EM_ATTITUDE_TYPE_ONE_HAND,					// 单手递接
	EM_ATTITUDE_TYPE_AKIMBO,					// 叉腰
	EM_ATTITUDE_TYPE_HAND_SON_HEAD,				// 双手抱头
	EM_ATTITUDE_TYPE_HAND_SON_CHEST,			// 双手抱胸
	EM_ATTITUDE_TYPE_HIT_WALL,					// 撞墙
	EM_ATTITUDE_TYPE_BEND,						// 弯腰
	EM_ATTITUDE_TYPE_SITTING,					// 坐状态
	EM_ATTITUDE_TYPE_STANDING,					// 站状态
	EM_ATTITUDE_TYPE_SIT_DOWN,					// 坐下
	EM_ATTITUDE_TYPE_STAND_UP,					// 起立
	EM_ATTITUDE_TYPE_ON_TABLE,					// 趴桌子
	EM_ATTITUDE_TYPE_LOOK_BACK,					// 回头
	EM_ATTITUDE_TYPE_DELIVERY,					// 传递
	EM_ATTITUDE_TYPE_GAZE_AROUND,				// 东张西望
	EM_ATTITUDE_TYPE_HAND_DOWN_PEEP,			// 手放桌下偷看
	EM_ATTITUDE_TYPE_PICK_UP,			        // 捡东西
	EM_ATTITUDE_TYPE_HAND_UP,			        // 举手
	EM_ATTITUDE_TYPE_WRITE,			            // 书写
	EM_ATTITUDE_TYPE_WAVE_HAND,					// 挥手
	EM_ATTITUDE_TYPE_HAND_POSE_OK,				// OK手势
	EM_ATTITUDE_TYPE_VICTORY,					// 比耶手势
	EM_ATTITUDE_TYPE_GOOD,						// 点赞手势
	EM_ATTITUDE_TYPE_HELP,						// 双手举过头顶(求救)
	EM_ATTITUDE_TYPE_TOUCH_CULTURAL_RELIC,		// 触摸文物
} EM_ATTITUDE_TYPE;

///@brief 姿态发生检测或屏蔽区域信息
typedef struct tagCFG_GENEAL_ATTITUDE_REGION_INFO
{
	BOOL					bEnable;						// 此区域是否起作用
	UINT					nRegionId;						// 区域id
	UINT					nRegionType;					// 区域类型: 1-检测区域 2-屏蔽区域
	UINT					nRegionDirect;					// 区域方向类型: 0-上方区域 1-下方区域 2-左方区域 3-右方区域 4-前方区域 5-后方区域
	int						nDetectRegionNum;				// 多边形个数
	CFG_POLYGON				stuDetectRegion[20];			// RegionId对应的多边形类型
	char					szResvered[252];				// 保留字节
}CFG_GENEAL_ATTITUDE_REGION_INFO;

///@brief EVENT_IVS_GENEAL_ATTITUDE(通用姿态行为事件)对应的规则配置
typedef struct tagCFG_GENEAL_ATTITUDE_INFO
{
	char					szRuleName[128];				// 规则名称,不同规则不能重名
	BOOL                    bRuleEnable;					// 规则使能
	int                     nObjectTypeNum;					// 相应物体类型个数
	...
	int						nDetectRegionNum;				// 检测区顶点数
	CFG_POLYGON				stuDetectRegion[20];			// 检测区
	UINT					nMinDuration;					// 最短持续时间, 单位:秒(5s-60s)
	UINT					nRecordInterval;                // 重复报警间隔, 单位:秒(0s-300s)
	UINT					nSensitivity;					// 灵敏度, 值越小灵敏度越低, 取值1-10
	UINT					nRateEnable;					// 是否激活动作评价功能，0-不激活，1-激活
	EM_ATTITUDE_TYPE		emAttitudeType;					// 姿态类型
	int						nRegionInfoNum;					// 姿态发生检测或屏蔽区域信息个数
	CFG_GENEAL_ATTITUDE_REGION_INFO		stuRegionInfo[16];	// 姿态发生检测或屏蔽区域信息

	char					szReserved[512];				// 保留字节
}CFG_GENEAL_ATTITUDE_INFO;
...
///@brief 周期模式配置
typedef struct tagCFG_RAINBRUSH_CYCLE
{
	char				szUnit[32];							/// 周期模式下，周期的单位
	int					nCycleLength;						/// 周期模式下，周期长度，单位由Unit字段决定
	int					nRunTime;							/// 周期模式下，雨刷运行的时长，单位：秒
	int					nCycleInterval;						/// 周期模式下，雨刷运行的间隔时间，单位：秒
	int					nSelfCleanTimes;					/// 周期模式下，一个周期内完成自清洁操作的次数
	char				szReserved[1024];					/// 保留字段
}CFG_RAINBRUSH_CYCLE;
...
typedef struct tagCFG_RAINBRUSH_INFO
{
	...
	CFG_RAINBRUSH_CYCLE	stuRainBrushCycle;						/// 周期模式配置，当CFG_RAINBRUSHMODE_INFO中EM_CFG_RAINBRUSHMODE_MODE为EM_CFG_RAINBRUSHMODE_MODE_CYCLE时有效
	int					nSelfCleanRunTime;						/// 手动模式下，一次自清洗操作雨刷运行的时长，单位：秒
	char				szReserved[1024];						/// 保留字节
}CFG_RAINBRUSH_INFO;
...
typedef struct tagCOILCONFIG_INFO
{
    ...
    int               nFlashSerialNum;               // 闪光灯序号	范围0~5，0表示不打开闪光灯
    int               nRedDirection;                 // 红灯方向	每个线圈对应的红灯方向：0-不关联,1-左转红灯,2-直行红灯,3-右转红灯,4-待左,5-待直,6-待右, 只在电警中有效
    int               nTriggerMode ;                 // 线圈触发模式	触发模式：0-入线圈触发1-出线圈触发
...
typedef struct tagDETECTOR_INFO
{
	...
	int				   nSnapMode;						 //抓拍模式 0-全部抓拍 1-超速抓拍 2-逆向抓拍
...
///@brief 报警联动信息扩展
typedef struct tagCFG_ALARM_MSG_HANDLE_EX
{
	int				nSnapshotTitleFontSize[256]; ///标题文本字体大小，单位像素,对应SnapshotTitle，个数与stuSnapshotTitle匹配。
	char			szReserved[1024]; /// 预留字节
}CFG_ALARM_MSG_HANDLE_EX;
...
typedef struct tagCFG_TRAFFICSNAPSHOT_INFO
{
    ...
    DWORD                    nVideoTitleMask1;					   // 从低位到高位分别表示：0-车标 1-红灯结束时间 2-设备制造商 3-小车低限速 4-大车低限速 5-小车高限速 6-大车高限速 7-设备工作模式 8-通用自定义 9-车道自定义 10-抓拍触发源 11-停车场区域12-车辆类型(面包车、轿车等等) 13-中车低限速 14-中车高限速 15-道路方向 16-GPS信息
	...
	CFG_ALARM_MSG_HANDLE_EX	 stuEventHandlerEx;						// 交通抓拍联动参数扩展
...
///@brief 普通配置本地GUI锁定策略
typedef struct tagCFG_LOCAL_POLICY_INFO
{
	BOOL			bLockLoginEnable;			// 登陆失败可尝试次数使能
	UINT				nCheckDuration;					// 清零周期，如果规定时间内登陆次数未超过可尝试次数，尝试次数清零
	int				nLockLoginTimes;					// 登陆失败可尝试次数
	int				nLoginFailLockTime;				// 登陆失败锁定时间
	char				szReserved[128];					// 保留字节
}CFG_LOCAL_POLICY_INFO;
...
typedef struct tagCFG_DEV_DISPOSITION_INFO
{
	...
	UINT						nCheckDuration;					// 清零周期，如果规定时间内登陆次数未超过可尝试次数，尝试次数清零
	BOOL					bUseLocalPolicy;					// 是否使用本地GUI锁定策略
	CFG_LOCAL_POLICY_INFO stuLocalPolicy;			// 本地GUI锁定策略
	char                     szMachineName1[256];    // 机器名称或编号
	BYTE		             bReserved[508];        // 保留字节
...
typedef struct tagTrafficEventCheckInfo
{
    ...
    BOOL                abPSRiseDetection;        // 是否携带人员起身检测信息
    EM_CHECK_TYPE       emPSRiseDetection;        // 人员起身检测检测类型

    BOOL                abFaceDetection;                // 是否携带目标检测信息
    EM_CHECK_TYPE       emFaceDetection;                // 目标检测检测类型
...
typedef struct tagViolationTimeSchedule
{
    ...
    BOOL                abFaceDetection;                // 是否携带目标检测信息
    TIME_SCHEDULE_INFO  stFaceDetection;                // 目标检测时间配置
...
///@brief 分辨率枚举,细分到具体分辨率
typedef enum tagEM_CAPTURE_SIZE_EX1
{
	EM_CAPTURE_SIZE_EX1_UNKNOWN = -1,     				  /// 未知
    EM_CAPTURE_SIZE_EX1_D1,                               /// D1   704*576(PAL)  704*480(NTSC),兼容WWxHH,下同
    EM_CAPTURE_SIZE_EX1_D1_PAL,                           /// 704*576(PAL)  
    EM_CAPTURE_SIZE_EX1_D1_NTSC,                          /// 704*480(NTSC)
    EM_CAPTURE_SIZE_EX1_HD1,                              /// HD1   352*576(PAL)  352*480(NTSC)
    EM_CAPTURE_SIZE_EX1_HD1_PAL,                          /// 352*576(PAL)  
    EM_CAPTURE_SIZE_EX1_HD1_NTSC,                         /// 352*480(NTSC)
    EM_CAPTURE_SIZE_EX1_BCIF,                             /// BCIF   704*288(PAL)  704*240(NTSC)
    EM_CAPTURE_SIZE_EX1_BCIF_PAL,                         /// 704*288(PAL)
    EM_CAPTURE_SIZE_EX1_BCIF_NTSC,                        /// 704*240(NTSC)
    EM_CAPTURE_SIZE_EX1_CIF,                              /// CIF    352*288(PAL)  352*240(NTSC)
    EM_CAPTURE_SIZE_EX1_CIF_PAL,                          /// 352*288(PAL)
    EM_CAPTURE_SIZE_EX1_CIF_NTSC,                         /// 352*240(NTSC)
    EM_CAPTURE_SIZE_EX1_QCIF,                             /// QCIF    176*144(PAL)  176*120(NTSC)
    EM_CAPTURE_SIZE_EX1_QCIF_PAL,                         /// 176*144(PAL)
    EM_CAPTURE_SIZE_EX1_QCIF_NTSC,                        /// 176*120(NTSC)
    EM_CAPTURE_SIZE_EX1_VGA,                              /// 640*480
    EM_CAPTURE_SIZE_EX1_QVGA,                             /// 320*240
    EM_CAPTURE_SIZE_EX1_SVCD,                             /// 480*480
    EM_CAPTURE_SIZE_EX1_QQVGA,                            /// 160*128
    EM_CAPTURE_SIZE_EX1_SVGA,                             /// 800*592
    EM_CAPTURE_SIZE_EX1_XVGA,                             /// 1024*768
    EM_CAPTURE_SIZE_EX1_WXGA,                             /// 1280*800
    EM_CAPTURE_SIZE_EX1_SXGA,                             /// 1280*1024  
    EM_CAPTURE_SIZE_EX1_WSXGA,                            /// 1600*1024  
    EM_CAPTURE_SIZE_EX1_UXGA,                             /// 1600*1200
    EM_CAPTURE_SIZE_EX1_WUXGA,                            /// 1920*1200
    EM_CAPTURE_SIZE_EX1_LTF,                              /// 240*192,ND1
    EM_CAPTURE_SIZE_EX1_720,                              /// 1280*720
    EM_CAPTURE_SIZE_EX1_1080,                             /// 1920*1080
    EM_CAPTURE_SIZE_EX1_1_3M,                             /// 1280*960
    EM_CAPTURE_SIZE_EX1_2M,                               /// 1872*1408,2_5M
    EM_CAPTURE_SIZE_EX1_5M,                               /// 3744*1408
    EM_CAPTURE_SIZE_EX1_3M,                               /// 2048*1536
    EM_CAPTURE_SIZE_EX1_5_0M,                             /// 2432*2050
    EM_CAPTURE_SIZE_EX1_1_2M,                             /// 1216*1024
    EM_CAPTURE_SIZE_EX1_1408_1024,                        /// 1408*1024
    EM_CAPTURE_SIZE_EX1_8M,                               /// 3296*2472
    EM_CAPTURE_SIZE_EX1_2560_1920,                        /// 2560*1920(5_1M)
    EM_CAPTURE_SIZE_EX1_960H,                             /// 960H    960*576(PAL) 960*480(NTSC)
    EM_CAPTURE_SIZE_EX1_960H_PAL,                         /// 960*576(PAL)
    EM_CAPTURE_SIZE_EX1_960H_NTSC,                        /// 960*480(NTSC)
    EM_CAPTURE_SIZE_EX1_960_720,                          /// 960*720
    EM_CAPTURE_SIZE_EX1_NHD,                              /// 640*360
    EM_CAPTURE_SIZE_EX1_QNHD,                             /// 320*180
    EM_CAPTURE_SIZE_EX1_QQNHD,                            /// 160*90
    EM_CAPTURE_SIZE_EX1_960_540,                          /// 960*540
    EM_CAPTURE_SIZE_EX1_640_352,                          /// 640*352
    EM_CAPTURE_SIZE_EX1_640_400,                          /// 640*400
    EM_CAPTURE_SIZE_EX1_320_192,                          /// 320*192    
    EM_CAPTURE_SIZE_EX1_320_176,                          /// 320*176
	EM_CAPTURE_SIZE_EX1_SVGA1,                            /// 800*600
	EM_CAPTURE_SIZE_EX1_2560_1440,						   /// 2560*1440
	EM_CAPTURE_SIZE_EX1_2304_1296,						   /// 2304*1296
	EM_CAPTURE_SIZE_EX1_2592_1520,						   /// 2592*1520
	EM_CAPTURE_SIZE_EX1_4000_3000,						   /// 4000*3000
	EM_CAPTURE_SIZE_EX1_2880_2880,						   /// 2880*2880
	EM_CAPTURE_SIZE_EX1_2880_2160,						   /// 2880*2160
	EM_CAPTURE_SIZE_EX1_2688_1520,						   /// 2688*1520
	EM_CAPTURE_SIZE_EX1_2592_1944,						   /// 2592*1944
	EM_CAPTURE_SIZE_EX1_3072_1728,						   /// 3072*1728
	EM_CAPTURE_SIZE_EX1_3072_2048,						   /// 3072*2048
	EM_CAPTURE_SIZE_EX1_3840_2160,						   /// 3840*2160
    EM_CAPTURE_SIZE_EX1_NR=255  
} EM_CAPTURE_SIZE_EX1;

///@brief 码流分辨率限制扩展
typedef struct tagNET_ENCODING_RESOLUTION_LIMIT_EX	 	 	 
{
	int 					nResolutionNum;			  /// 分辨率个数
    EM_CAPTURE_SIZE_EX1      emResolution[10];          /// 分辨率              
    char                	szReserved[1024];               /// 保留字节	 	 	 
}NET_ENCODING_RESOLUTION_LIMIT_EX;	 
...
typedef struct tagCFG_PRODUCT_DEFINITION_INFO
{
	...
	int					    nEncodingResolutionLimitNum;						///码流分辨率组合限制个数
    NET_ENCODING_RESOLUTION_LIMIT_EX				stuEncodingResolutionLimit[10];	/// 码流分辨率限制，限制界面和功能模块只能使用指定范围之内的分辨率，前提是设备支持该分辨率。是一个二维数组,第一级对应不同的码流,第二级对应支持的分辨率集合。二级数组为空表示不做限制，使用所有设备能提供的分辨率。
	char										szReserved[1024];				/// 保留字节
...
typedef struct tagCFG_IPSERVER_STATUS
{
	...
	char                   szSupportServer[MAX_SEVER_NUM][MAX_NAME_LEN];  // 提供的服务名称 Svr Svrd(SVR守护服务) DataBase DataBased(DataBase守护服务) NtpServer NtpServerd(NtpServer守护服务)  Samba Nfs Ftp iScsi 
...
{
	BOOL								bEnable;					// 使能配置
	UINT								nFaceAlgorithmThreshold;	// 目标算法阀值
	CFG_VIDEO_ALGORITHMTYPE_ALGORITHM	stuFaceAlgorithm;			// 目标算法百分占比，总100
...
typedef struct tagCFG_VIDEO_IN_OPTIONS
{
	...
	BYTE				byWhiteBalance;			// 白平衡, 0-"Disable", 1-"Auto", 2-"Custom", 3-"Sunny", 4-"Cloudy", 5-"Home", 6-"Office", 7-"Night", 8-"HighColorTemperature", 9-"LowColorTemperature", 10-"AutoColorTemperature", 11-"CustomColorTemperature"
																// 12-"Indoor", 13-"Outdoor", 14-"ATW", 15-"Manual", 16-"AutoOutdoor", 17-"SodiumAuto", 18-"Sodium", 19-"Track", 20-"AutoTrack", 21-"ManualDatum", 22-"PartWhiteBalance", 23-"Natural", 24-"StreetLamp"
	BYTE				byColorTemperature;		// 色温等级, 白平衡为"CustomColorTemperature"模式下有效
	bool				bMirror;				// 镜像
	bool				bFlip;					// 翻转
	bool				bIrisAuto;				// 自动光圈
...
typedef struct tagCFG_CFG_VIDEOSATA_SUMMARY_INFO
{
	...
	UINT					nEnteredTotalInTimeSection;			// 自动清零时间段内的人数（IPC专用，如果不执行clearSectionStat操作，等同于Today人数）
	UINT					nExitedTotalInTimeSection;			// 自动清零时间段内的人数（IPC专用，如果不执行clearSectionStat操作，等同于Today人数）
...
///@brief 重置镜头电机的结果
typedef enum EM_RESET_RESULT
{
	EM_RESET_RESULT_UNKNOW,        	// 未知
    EM_RESET_RESULT_SUCCESS,        // 重置成功
	EM_RESET_RESULT_FAILED,        	// 重置失败
}EM_RESET_RESULT;
...
typedef struct tagCFG_CAP_FOCUS_STATUS
{
	...
	UINT				nZoomMotorSteps;						// 变倍电机总步长，对采用步进电机的设备有效
	UINT				nFocusMotorSteps;						// 聚焦电机总步长，对采用步进电机的设备有效
	EM_RESET_RESULT		emResetResult;							// 重置镜头电机的结果，只在重置操作时有效
	UINT				nFocusMode;								// 聚焦模式: 1 混合，即普通聚焦, 2 人脸，即景深聚焦：针对人脸, 无此字段为普通聚焦
	char				szReserved[256];						// 预留字节
...
typedef struct AV_CFG_VideoWidgetCustomTitle 
{
	...
	AV_BOOL				bUpdate;						// 是否需要设备端更新叠加内容 true:更新  false:不更新
...
typedef enum AV_CFG_LanguageType
{
	...
	AV_CFG_Language_Kazakh,								// 哈萨克语, "Kazakh"
	AV_CFG_Language_Bengali,							// 孟加拉语, "Bengali"
	AV_CFG_Language_Gujarati,							// 古吉拉特语, "Gujarati"
	AV_CFG_Language_Hindi,								// 印地语, "Hindi"
    AV_CFG_Language_Tamil,								// 泰米尔语, "Tamil"
	AV_CFG_Language_Malayalam,							// 马拉雅拉姆语, "Malayalam"
	AV_CFG_Language_Telugu,								// 泰卢固语, "Telugu"
	AV_CFG_Language_Kannada,							// 卡纳达语, "Kannada"
	AV_CFG_Language_Marathi,							// 马拉地语, "Marathi"
}AV_CFG_LanguageType;
...
typedef struct AV_CFG_AutoMaintain 
{
	...
	AV_int32			nAutoShutDelay;				// 车载延时关机时间[0,65536]分钟，（ACC断电后开始计数）
}AV_CFG_AutoMaintain;
...
typedef struct AV_CFG_MonitorWallTVOut
{
	...
	int				nPosX;								/// 相对物理坐标系下水平方向起始位置
	int				nPosY;								/// 相对物理坐标系下垂直方向起始位置
...
typedef struct AV_CFG_MonitorWall
{
	...
	int					nCoordinateType;					/// 电视墙坐标系类型, 0:未知, 1:8192相对坐标系, 2:物理坐标系
}AV_CFG_MonitorWall;
...
typedef struct tagAV_CFG_TemperatureAlarm
{
	...
	AV_CFG_EventHandler stuEventHandler;					// 报警联动
	UINT				nReportInterval;							//报警时间间隔。（单位：秒）
...
typedef struct tagCFG_SERVER_INFO
{
	int                 nPort;                             // 服务器端口号
	char                szAddress[MAX_ADDRESS_LEN];        // IP地址或网络名
	BOOL				abEnable;							// bEnable 是否有效
	BOOL				bEnable;							// 主动注册使能
	char				szReserved[64];						// 预留字节
}CFG_SERVER_INFO;
...
typedef enum tagSUPPORT_FOCUS_MODE
{
	...
	ENUM_SUPPORT_FOCUS_FACE       = 4,                // 看清目标模式     
...
///@brief 风扇能力
typedef struct tagCFG_PTZ_FAN_CAPS
{
	BOOL					bSupport;					/// 是否支持风扇控制
	BOOL					bClearFogSupport;			/// 是否支持除雾
	BOOL					bFanPowerSupport;			/// 是否支持风扇电源管理
	char					szReserved[1020];			/// 预留字节
}CFG_PTZ_FAN_CAPS;

///@brief 视窗加热器能力
typedef struct tagCFG_WINDOW_HEATER_CAPS
{
	BOOL					bSupport;					/// 是否支持视窗加热
	int						nManualControlPeriod[2];	/// 手动开启时间范围，[最小时间，最大时间]，单位分钟
	int						nManualDefaultPeriod;		/// 手动默认开启时间，不同的设备默认值不同，需要同热设计确认，单位分钟
	BOOL					bIsOpposeLight;				/// 是否与补光灯功能互斥
	char					szReserved[1020];			/// 预留字节
}CFG_WINDOW_HEATER_CAPS;
...
typedef struct tagCFG_PTZ_PROTOCOL_CAPS_INFO
{
	...
	CFG_PTZ_FAN_CAPS				stuPtzFan;					/// 风扇能力
	CFG_WINDOW_HEATER_CAPS			stuWindowHeater;			/// 视窗加热器能力
...
typedef struct tagCFG_RTMP_INFO
{
	...
	char		szCustomPath[MAX_ADDRESS_LEN];				// 路径名
...
///@brief 合成通道各个窗口的信息
typedef struct tagCFG_COMPOSE_CHANNEL_WINDOW
{
	int				nChannel;					/// 视频通道号
	CFG_RECT		stuPosition;				/// 窗口在屏幕中的位置，使用8192坐标系，当 Split等于PIPn模式时，该字段为必填字段
	char			szType[16];					/// 窗口类型。当合成通道为画中画模式时，该字段用来表明画面的z轴顺序。主画面取值为Main%d，辅画面取值为Extra%d，%d为数字，从1开始
	UINT			nStreamType;				/// 码流类型, -1:未知, 0:主码流, 1:辅码流
	char			szReserved[1024];			/// 预留字节
}CFG_COMPOSE_CHANNEL_WINDOW;
...
typedef struct tagCFG_COMPOSE_CHANNEL 
{
	...
	char			szPlanName[16];				/// 合成通道使用的预案
	int				nAudioSrcNum;				/// 合成通道的音频源个数
	int				nAudioSrc[32];				/// 合成通道的音频源
	char			szAudioType[16];			/// 音频源的编码类型
	int				nWindowNum;					/// 合成通道窗口个数
	CFG_COMPOSE_CHANNEL_WINDOW	stuWindow[16];	/// 合成通道各个窗口的信息
	char			szReserved[1024];			/// 预留字节
} CFG_COMPOSE_CHANNEL;
...
///@brief 强光抑制模式，当背光模式为GlareInhibition时有效
typedef enum tagEM_GLARE_INHIBITION_MODE
{
	EM_GLARE_INHIBITION_UNKONW,		// 未知模式
	EM_GLARE_INHIBITION_DEFAULT,	// 默认模式
	EM_GLARE_INHIBITION_FPGA,		// Fpga强光抑制模式
} EM_GLARE_INHIBITION_MODE;
...
typedef struct tagCFG_VIDEOIN_BACKLIGHT_BASE
{
	...
	char							szName[16];				// 场景名称
	EM_GLARE_INHIBITION_MODE		emGlareInhibitionMode;	// 强光抑制模式, emMode == EM_BACKLIGHT_GLAREINHIBITION时有效
	int								nGlareInSensitivity;	// FPGA模式下强光抑制灵敏度, 范围[0-100]
	int								nGainMinBLC;			// FPGA模式下强光抑制增益下限, 范围[0-100], 且GainMinBLC <= GainMaxBLC
	int								nGainMaxBLC;			// FPGA模式下强光抑制增益上限, 范围[0-100], 且GainMinBLC <= GainMaxBLC
	double							dExposureMinBLC;		// FPGA模式下强光抑制曝光时间下限, 单位ms, 范围[0.1-40.0], ExposureMinBLC <= ExposureMaxBLC
	double							dExposureMaxBLC;		// FPGA模式下强光抑制曝光时间上限, 单位ms, 范围[0.1-40.0], ExposureMinBLC <= ExposureMaxBLC
	UINT							nWideDynamicRangeMode;	// 宽动态模式, 0：关闭 1：手动调节 2：自动调节
	char							szReserved[252];		// 预留字节
...
typedef enum tagCFG_DOOR_OPEN_METHOD
{
    ...
    CFG_DOOR_OPEN_METHOD_FINGERPRINTONLY                = 7,    // 仅信息开锁
    CFG_DOOR_OPEN_METHOD_PWD_OR_CARD_OR_FINGERPRINT     = 8,    // 密码或刷卡或信息开锁
    CFG_DOOR_OPEN_METHOD_PWD_AND_CARD_AND_FINGERPINT    = 9,    // 密码+刷卡+信息组合开锁
    CFG_DOOR_OPEN_METHOD_PWD_AND_FINGERPRINT            = 10,   // 密码+信息组合开锁
    CFG_DOOR_OPEN_METHOD_CARD_AND_FINGERPRINT           = 11,   // 刷卡+信息开锁
    CFG_DOOR_OPEN_METHOD_MULTI_PERSON                   = 12,   // 多人开锁
    CFG_DOOR_OPEN_METHOD_FACEIDCARD						= 13,   // 人证对比
    CFG_DOOR_OPEN_METHOD_FACEIDCARD_AND_IDCARD			= 14,	// 证件+ 人证比对
    CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FINGER	= 15,	// 人证比对或刷卡或信息
    CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_OR_CARD_OR_FINGER	= 16,	// (证件+认证比对)或刷卡或信息
    CFG_DOOR_OPEN_METHOD_USERID_AND_PWD                 = 17,   // UserID+密码
    CFG_DOOR_OPEN_METHOD_FACE_ONLY                      = 18,   // 只允许目标开锁
    CFG_DOOR_OPEN_METHOD_FACE_AND_PWD                   = 19,   // 目标+密码开锁
    CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_PWD            = 20,   // 信息+密码开锁
    CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_FACE           = 21,   // 信息+目标开锁
    CFG_DOOR_OPEN_METHOD_CARD_AND_FACE                  = 22,   // 刷卡+目标开锁
    CFG_DOOR_OPEN_METHOD_FACE_OR_PWD                    = 23,   // 目标或密码开锁
    CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_PWD             = 24,   // 信息或密码开锁
    CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_FACE            = 25,   // 信息或目标开锁
    CFG_DOOR_OPEN_METHOD_CARD_OR_FACE                   = 26,   // 刷卡或目标开锁
    CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT            = 27,   // 刷卡或信息开锁
    CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_FACE_AND_PWD   = 28,   // 信息+目标+密码开锁
    CFG_DOOR_OPEN_METHOD_CARD_AND_FACE_AND_PWD          = 29,   // 刷卡+目标+密码开锁
    CFG_DOOR_OPEN_METHOD_CARD_AND_FINGERPRINT_AND_PWD   = 30,   // 刷卡+信息+密码开锁
    CFG_DOOR_OPEN_METHOD_CARD_AND_PWD_AND_FACE          = 31,   // 卡+信息+目标组合开锁
    CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_FACE_OR_PWD     = 32,   // 信息或目标或密码
    CFG_DOOR_OPEN_METHOD_CARD_OR_FACE_OR_PWD               = 33,   // 卡或目标或密码开锁
    CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_FACE    = 34,   // 卡或信息或目标开锁
    CFG_DOOR_OPEN_METHOD_CARD_AND_FINGERPRINT_AND_FACE_AND_PWD  = 35,   // 卡+信息+目标+密码组合开锁 
    CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_FACE_OR_PWD     = 36,   // 卡或信息或目标或密码开锁
    CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_OR_CARD_OR_FACE    = 37,   //(证件+人证比对)或 刷卡 或 目标
    CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FACE        = 38,   // 人证比对 或 刷卡(二维码) 或 目标
	CFG_DOOR_OPEN_METHOD_CARDANDPWD_OR_FINGERPRINTANDPWD   = 39,   // (卡+密码）或（信息+密码）默认为2
	CFG_DOOR_OPEN_METHOD_PHOTO_OR_FACE					= 40,	//	人证(照片)或目标
	CFG_DOOR_OPEN_METHOD_FINGERPRINT					= 41,	//	人证(信息)
	CFG_DOOR_OPEN_METHOD_PHOTO_AND_FINGERPRINT			= 42,	//	人证(照片+信息)
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_FACE_OR_PASSWORD = 43,	// 人证或刷卡或信息或目标或密码
	CFG_DOOR_OPEN_METHOD_MULTI_USER_TYPE				= 44,	// 多用户类型
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_HEALTHCODE		= 45,	// 人证或健康码
	CFG_DOOR_OPEN_METHOD_IRIS_ONLY						= 46,	// 只允许眼睛开锁
	CFG_DOOR_OPEN_METHOD_IRIS_AND_PWD					= 47,	// 眼睛+密码组合开锁 
	CFG_DOOR_OPEN_METHOD_FACE_AND_IRIS					= 48,	// 目标+眼睛组合开锁 
	CFG_DOOR_OPEN_METHOD_CARD_AND_IRIS					= 49,	// 卡+眼睛组合开锁
	CFG_DOOR_OPEN_METHOD_IRIS_OR_PWD					= 50,	// 眼睛或密码开锁
	CFG_DOOR_OPEN_METHOD_FACE_OR_IRIS					= 51,	// 目标或眼睛开锁
	CFG_DOOR_OPEN_METHOD_CARD_OR_IRIS					= 52,	// 卡或眼睛开锁 
	CFG_DOOR_OPEN_METHOD_FACE_AND_IRIS_AND_PWD			= 53,	// 目标+眼睛+密码组合开锁 
	CFG_DOOR_OPEN_METHOD_CARD_AND_FACE_AND_IRIS			= 54,	// 卡+目标+眼睛组合开锁 
	CFG_DOOR_OPEN_METHOD_CARD_AND_IRIS_AND_PWD			= 55,	// 卡+眼睛+密码组合开锁 
	CFG_DOOR_OPEN_METHOD_FACE_OR_IRIS_PWD				= 56,	// 目标或眼睛或密码开锁 
	CFG_DOOR_OPEN_METHOD_CARD_OR_FACE_IRIS				= 57,	// 卡或目标或眼睛开锁 
	CFG_DOOR_OPEN_METHOD_CARD_OR_IRIS_PWD				= 58,	// 卡或眼睛或密码开锁 
	CFG_DOOR_OPEN_METHOD_CARD_AND_FACE_IRIS_PWD			= 59,	// 卡+目标+眼睛+密码组合开锁 
	CFG_DOOR_OPEN_METHOD_CARD_OR_FACE_OR_IRIS_PWD		= 60,	// 卡或目标或眼睛或密码开锁
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD				= 61,	// 人证或刷卡
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_OWD				= 62,	// 人证或密码
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_FINGERPRINT		= 63,	// 人证或信息
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_PWD		= 64,	// 人证或刷卡或密码
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_FINGERPRINT_OR_PWD	= 65, //人证或信息或密码
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_FINGERPRINT_OR_FACE  = 66, //人证或信息或目标
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_PWD		= 67,	// 人证或刷卡或信息或密码
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_FACE		= 68,	// 人证或刷卡或信息或目标	
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_PWD_OR_FINGERPRINT_OR_FACE		= 69,	// 人证或密码或信息或目标	
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_CARD		= 70,	// (证件+人证比对)或刷卡
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_PWD		= 71,	// (证件+人证比对)或密码
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_FINGERPRINT		= 72,	// (证件+人证比对)或信息
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_FACE		= 73,	// (证件+人证比对)或目标
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_CARD_OR_PWD		= 74,	// (证件+人证比对)或刷卡或密码
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_FINGERPRINT_OR_PWD		= 75,	// (证件+人证比对)或信息或密码
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_FINGERPRINT_OR_FACE		= 76,	// (证件+人证比对)或信息或目标
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_PWD		= 77,	// (证件+人证比对)或信息或目标或密码
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_FACE		= 78,	// (证件+人证比对)或刷卡或信息或目标
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_PWD_OR_FINGERPRINT_OR_FACE		= 79,	// (证件+人证比对)或密码或信息或目标
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_PWD_OR_FINGERPRINT_OR_CARD_OR_FACE		= 80,	// (证件+人证比对)或信息或目标	
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_PWD_OR_CARD_OR_FACE		= 81,	// 人证或密码或刷卡或目标
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_PWD_AND_CARD_AND_FACE		= 82,	//  (证件+人证)或密码、刷卡、目标
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_PWD_OR_FACE		= 83,	// 人证或密码或目标
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_AND_FACEIDCARD_OR_PWD_OR_FACE		= 84,	// (证件+人证比对)或目标
	CFG_DOOR_OPEN_METHOD_MULTI_COMBINATIONS				= 85,	// 多组合开门
	CFG_DOOR_OPEN_METHOD_BLUETOOTHCARD					= 86,	// 蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_CARD_OR_BLUETOOTHCARD			= 87,	// 卡或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_BLUETOOTHCARD	= 88,	// 信息或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_PWD_OR_BLUETOOTHCARD			= 89,	// 密码或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_BLUETOOTHCARD			= 90,	// 卡或信息或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_CARD_OR_PWD_OR_BLUETOOTHCARD					= 91,	// 卡或密码或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_PWD_OR_BLUETOOTHCARD			= 92,	// 信息或密码或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_PWD_OR_BLUETOOTHCARD	= 93,	// 卡或信息或密码或蓝牙卡开门
	CFG_DOOR_OPEN_METHOD_PALMPRINT_ONLY = 94,					// 只允许掌纹开锁 
	CFG_DOOR_OPEN_METHOD_PALMPRINT_AND_PWD	= 95,				// 掌纹+密码组合开锁
	CFG_DOOR_OPEN_METHOD_FACE_AND_PALMPRINT	= 96,				// 人脸+掌纹组合开锁
	CFG_DOOR_OPEN_METHOD_CARD_AND_PALMPRINT	= 97,				// 卡+掌纹组合开锁
	CFG_DOOR_OPEN_METHOD_PALMPRINT_OR_PWD	= 98,				// 掌纹或密码开锁
	CFG_DOOR_OPEN_METHOD_FACE_OR_PALMPRINT	= 99,				// 人脸或掌纹开锁
	CFG_DOOR_OPEN_METHOD_CARD_OR_PALMPRINT	= 100,				// 卡或掌纹开锁 
	CFG_DOOR_OPEN_METHOD_FACE_AND_PALMPRINT_AND_PWD	= 101,		// 人脸+掌纹+密码组合开锁
	CFG_DOOR_OPEN_METHOD_CARD_AND_FACE_AND_PALMPRINT	= 102,	// 卡+人脸+掌纹组合开锁
	CFG_DOOR_OPEN_METHOD_CARD_AND_PALMPRINT_AND_PWD	= 103,		// 卡+掌纹+密码组合开锁
	CFG_DOOR_OPEN_METHOD_FACE_OR_PALMPRINT_OR_PWD	= 104,		// 人脸或掌纹或密码开锁
	CFG_DOOR_OPEN_METHOD_CARD_OR_FACE_OR_PALMPRINT	= 105,		// 卡或人脸或掌纹开锁
	CFG_DOOR_OPEN_METHOD_CARD_OR_PALMPRINT_OR_PWD	= 106,		// 卡或掌纹或密码开锁
	CFG_DOOR_OPEN_METHOD_CARD_AND_FACE_AND_PALMPRINT_AND_PWD	= 107,	// 卡+人脸+掌纹+密码组合开锁
	CFG_DOOR_OPEN_METHOD_CARD_OR_FACE_OR_PALMPRINT_OR_PWD	= 108,		// 卡或人脸或掌纹或密码开锁
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD 		= 109,		// 证件开锁
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_OR_CARD_OR_FACE_OR_PALMPRINT_OR_PWD = 110,	// 证件或卡或人脸或指纹或密码开锁
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_IRIS = 111,			// 指纹+虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_IRIS = 112,				// 指纹或虹膜
	CFG_DOOR_OPEN_METHOD_CARD_AND_FINGERPRINT_AND_IRIS = 113,	// 卡+指纹+虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_FACE_AND_IRIS = 114,	// 指纹+人脸+虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_IRIS_AND_PWD = 115,	// 指纹+虹膜+密码
	CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_IRIS = 116,		// 卡或指纹或虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_FACE_OR_IRIS = 117,		// 指纹或人脸或虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_IRIS_OR_PWD = 118,		// 指纹或虹膜或密码
	CFG_DOOR_OPEN_METHOD_CARD_AND_FINGERPRINT_AND_IRIS_AND_PWD = 119,		// 卡+指纹+虹膜+密码
	CFG_DOOR_OPEN_METHOD_CARD_AND_FINGERPRINT_AND_FACE_AND_IRIS = 120,		// 卡+指纹+人脸+虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_AND_FACE_AND_IRIS_AND_PWD = 121,		// 指纹+人脸+虹膜+密码
	CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_IRIS_OR_PWD = 122,			// 卡或指纹或虹膜或密码
	CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_FACE_OR_IRIS = 123,			// 卡或指纹或人脸或虹膜
	CFG_DOOR_OPEN_METHOD_FINGERPRINT_OR_FACE_OR_IRIS_OR_PWD = 124,			// 指纹或人脸或虹膜或密码
	CFG_DOOR_OPEN_METHOD_CARD_OR_FINGERPRINT_OR_FACE_OR_IRIS_OR_PWD = 125,	// 卡或指纹或人脸或虹膜或密码
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_FACE_OR_IRIS_OR_PWD = 126,			// 人证或卡或指纹或人脸或虹膜或密码
	CFG_DOOR_OPEN_METHOD_FACEIPCARDANDIDCARD_OR_CARD_OR_FINGERPRINT_OR_FACE_OR_IRIS_OR_PWD = 127,	// (身份证+人证比对)或卡或指纹或人脸或虹膜或密码
...
///@brief 门禁的AB互锁的组
typedef struct tagCFG_ABLOCK_DOOR_INFO_EX 
{
    int                 nDoorCount;                                  /// 有效互锁门的个数
    UINT                 nDoor[32];       						/// 互锁的门的通道号
    char 				szReserved[68];							/// 预留字节
}CFG_ABLOCK_DOOR_INFO_EX;

///@brief 门禁的AB互锁功能, 多个门通道，只有其他B通道都关闭，唯一的A通道才打开
typedef struct tagCFG_ABLOCK_INFO_EX 
{
    BOOL                bEnable;                                /// 使能
    int                 nDoorsCount;                                 /// 有效互锁组数
    CFG_ABLOCK_DOOR_INFO_EX stuDoors[8];    						/// 互锁组信息
    char 				szReserved[64];							/// 预留字节
}CFG_ABLOCK_INFO_EX;
...
typedef struct tagCFG_ACCESS_GENERAL_INFO
{
	...
	UINT						nFloorOffset;				// 负楼层偏移：0~15

	BOOL					bButtonExitEnable;		// 开门按钮使能
	BOOL					bCustomMethodEnable;	// 是否启用个性化开门方式
	int						nMethod;							//-1:未知
																		//0: 只允许密码开锁
																		//1: 只允许刷卡开锁
																		//2: 密码或刷卡开锁
																		...
																		//8: 密码+信息组合开锁
																		//9: 刷卡+信息组合开锁
	BOOL					bCheckSensorBeforeLock;	// 闭锁前是否检测门磁信号
	BOOL					bUnlockRecordUpload;	// 开锁记录是否上传
	UINT						nCheckSensorTime;		// 门磁检测时间
	char						szSendCardPassword[64]; // 发卡密码
	BOOL					bRecordEnable;	// 录像使能
	BOOL					bCallLiftEnable;	// 呼梯使能
	char						szCallLiftType[32]; //呼梯的协议类型
	BOOL					bSnapshotUpload; //抓图上传使能
	BOOL					bAutoSnapshot; //呼叫自动抓拍使能
	char						szOpenDoorCommand[32]; //Dtmf 开门命令
	char						szOpenDoorSecondCommand[32]; //Dtmf 第二个开门命令
	int						nAccessProtocol;			//门禁协议 0:未知 1 :本机开关量控制 2:门禁协议(串口协议) 3:门禁udp开锁 4:私有串口
	int						nAccessProtocolSecond;			// 0:未知 1 :开本地锁 2:485外接的锁
	UINT						nEntranceLockChannel;				// 门禁控制器下的子通道nAccessProtocol为2有效
	int						nUnlockRecordTypeNum;			// 开锁方式数量
	int						nUnlockRecordType[32];			// 开锁方式 0：未知 1:Password 2:Button 3:FgerPrint 4:QRCode 5:Card 6:Remote 7:BlueTooth 8:Fce 9:DTMF 10:QRCodeEx 11:CustomPassword
	char						szICKeyValue[2][32];					// 密钥数组
	int						nICKeyValueNum;						// 密钥数量
	BOOL					bICKeyEnable;							// 检验IC卡密码功能使能
	BOOL					bABLockInfo;						/// 控制是否使用AB互锁信息扩展
	CFG_ABLOCK_INFO_EX*			pstuABLockInfoEx;				/// AB互锁信息扩展,由用户申请内存,大小为sizeof(CFG_ABLOCK_INFO_EX)
	char						szReserved[696 - CFG_POINTERSIZE];			// 保留字节
...
typedef struct tagCFG_DOOROPEN_TIMESECTION_INFO
{
	...
	BOOL					abOutDoorMethodEnable;	// 是否支持使能出门开门方式
	BOOL					bOutDoorMethodEnable;	// 是否使能出门开门方式（若使能，出门和进门模式可以分别配置）
	UINT					nOutDoorMethod;			// 时间段内出门开门方式(bOutDoorMethodEnable为TRUE时有效)
	char					szReserved[36];			// 保留字段
...
typedef enum CFG_ACCESS_FIRSTENTER_STATUS
{
	...
	ACCESS_FIRSTENTER_STATUS_NORMAL		// Normal-首卡权限验证通过后，其他用户才能刷卡(信息等)验证通过
}CFG_ACCESS_FIRSTENTER_STATUS;

///@brief 首卡开门信息
typedef struct tagCFG_ACCESS_FIRSTENTER_INFO
{
	BOOL							bEnable;	// 在指定的时间,只有拥有首卡权限的用户验证通过后，其他的用户才能刷卡(信息等)进入，TRUE 使能 FALSE 关闭
	CFG_ACCESS_FIRSTENTER_STATUS	emStatus;	// 首卡权限验证通过后的门禁状态
	int								nTimeIndex;	// 需要首卡验证的时间段, 值为通道号
}CFG_ACCESS_FIRSTENTER_INFO;

///@brief 首卡开门信息（拓展）与 CFG_ACCESS_FIRSTENTER_INFO 配合使用，是其拓展
typedef struct tagCFG_ACCESS_FIRSTENTER_INFO_EX
{
	UINT				nHolidayScheduleID;					// 验证假日计划,需要首卡验证的假日计划
	char				szReserved[124];					// 预留字节
}CFG_ACCESS_FIRSTENTER_INFO_EX;
...
///@brief 开门远程验证扩展
typedef struct tagCFG_AUTO_REMOTE_CHECK_INFO_EX
{
	UINT				nHolidayScheduleID;				// 远程验证假日计划, 需要远程验证的假日计划, 值为AccessHolidaySchedule索引
	char				szResvered[60];					// 保留字节
}CFG_AUTO_REMOTE_CHECK_INFO_EX;
...
typedef struct tagCFG_ACCESS_EVENT_INFO
{
    ...
    bool				abSpecialDaysOpenAlwaysTime;	// 是否支持SpecialDaysOpenAlwaysTime字段
	bool				abSpecialDaysCloseAlwaysTime;	// 是否支持SpecialDaysCloseAlwaysTime字段
	...
	UINT				nSensorType;						// 门磁类型, 0: 未知, 1: 常开型门磁, 2: 常闭型门磁
	CFG_AUTO_REMOTE_CHECK_INFO_EX		stuAutoRemoteCheckEx;		// 自动远程开门扩展信息
	BOOL				bIsSpecialDaysAuthenticationTimeValid;		// 门禁假日正常鉴权时间段是否有效
	UINT				nSpecialDaysAuthenticationTime;		// 门禁假日正常鉴权时间段,值为SpecialDaysSchedule索引.假日常开大于假日常闭大于假日鉴权大于普通常开大于普通常闭
	BOOL				bIsOpenDoorGroupTimeOutValid;		// 多人组合验证超时时间是否有效
	UINT				nOpenDoorGroupTimeOut;				// 多人组合验证超时时间，配套多人组合开门使用，单位：秒[10-60]，默认20
	CFG_ACCESS_FIRSTENTER_INFO_EX	stuFirstEnterInfoEx;	// 首卡开门信息(拓展)，与stuFirstEnterInfo搭配使用
	BOOL				bIsDoorNotClosedReaderAlarmTypeValid;		// 门未关超时报警联动读卡器蜂鸣类型是否有效
	UINT				nDoorNotClosedReaderAlarmType;		// 门未关超时报警联动读卡器蜂鸣类型 ,0 : 报警后读卡器按照设定时长蜂鸣，时间到停止蜂鸣,1 : 报警后读卡器一直蜂鸣，直到门关上为止
	BOOL				bAutoRemoteHolidayScheduleIDUnenable;			// 不下发远程验证假日计划字段，(仅下发有效)
	BOOL				bFirstEnterHolidayScheduleIDUnenable;			// 不下发首卡验证假日计划字段，(仅下发有效)
	BOOL				abOutDoorMethodEnable;							// 是否支持使能出门开门方式
	BOOL				bOutDoorMethodEnable;							// 是否使能出门开门方式（若使能，出门和进门模式可以分别配置）
	UINT				nOutDoorMethod;									// 时间段内出门开门方式(bOutDoorMethodEnable为TRUE时有效)
	UINT				nCardStoreFormat;								// 设备卡片数据存储进制，1:十六进制，2:十位十进制，0时不下发该字段
	UINT				nRepeatCardEnterTime;							/// 刷卡重复验证时间，卡片在该时间内无法重复刷第二次。0表示该字段功能不生效。
	char				szResvered[844];								// 保留字节
...
typedef struct tagCFG_ACCESS_TIMESCHEDULE_INFO 
{
	...
	BOOL					bIsTimeExValid;									// 是否使用stuTimeEx,为TRUE时stuTime字段无效,仅下发时使用,获取时stuTime与stuTimeEx均有效
	CFG_TIME_SECTION		stuTimeEx[7][6];								// 刷卡时间段（拓展）		
	BOOL					bTimeScheduleConsumptionTimesEx;				// nTimeScheduleConsumptionTimesEx字段是否有效
	int						nTimeScheduleConsumptionTimesEx[8][6];			// 每个时间段可消费的次数
																			// 第一维：前7个元素代表每周7天，第8个元素对应节假日
																			// 7个元素中第一个是星期日，第二个是星期一，以此类推
																			// 第二维：每天最多6个时间段
																			// （设备只支持7*4，数组大小和stuTime保持一致）
	BOOL					bIsTimeEx2Valid;								// 是否使用stuTimeEx2,仅下发时使用,优先bIsTimeExValid,需要bIsTimeExValid置为FALSE
	int 					nMaxTimeEx2Count;								// 需要获取/设置的时间段数量，既用户分配并赋给pstuTimeEx2的CFG_TIME_SECTION 结构个数,下发时必须是10的倍数
	int						nRetTimeEx2Count;								// 实际获取到的时间段数量，设置配置时此成员无意义
	CFG_TIME_SECTION*		pstuTimeEx2;									// 刷卡时间段扩展2，由用户申请内存,大小为sizeof(CFG_TIME_SECTION)*nMaxTimeEx2Count
	char					szReserved[816 - CFG_POINTERSIZE];				// 预留字节
...
{
	BOOL			bEnable;								// 使能
	char			szProtocol[MAX_PROTOCOL_NAME_LEN];		// 协议类型
...
///@brief 报警输出时间段控制
typedef struct tagNET_ALARMOUT_TIME_SCHEDULE_INFO
{
	BOOL				bEnable;									/// 分时段报警输出使能
	char				szReserved[1020];							/// 预留字节 
}NET_ALARMOUT_TIME_SCHEDULE_INFO;
...
typedef struct tagCFG_ALARMOUT_INFO		// =>CFG_CMD_ALARMOUT
{
	...
	NET_ALARMOUT_TIME_SCHEDULE_INFO		stuTimeSchedule;			// 报警输出时间段控制信息
	char								szReserved[1024];			// 预留字节		
}CFG_ALARMOUT_INFO;
...
typedef struct tagCFG_CAP_EVENTMANAGER_INFO
{
	...
	DWORD				nSupportDisableLinkageTimeSection;				  // 支持周期撤防
	                                                                      // bit0表示支持周期撤防
	int					nSupportDelayUploadSeqFileEventsNum;				// 支持先报警后延时上传文件的事件个数
	char				szSupportDelayUploadSeqFileEvents[32][32];			// 支持先报警后延时上传文件的事件集合
	BOOL				bSupportTransferOfflineACK;							// 是否支持事件ack续传
	BOOL  				bSupportReEnableLinkage;							// 是否支持一键撤防后自动布防EventHandlerParam
	char				szResvered[1016];									// 保留字节
...
typedef struct tagTRAFFIC_TRANSFER_OFFLINE_INFO
{
	...
	UINT			nTransferTimes[2];										// 重传次数（限制平台客户端最大2个），表示同一张图片最大重传次数，默认值为0表示不限制重传次数
	UINT			nTransferTimesNum;										// 重传次数有效个数
	char			szReserved[1020];										// 保留字节
...
{
    int                 nSceneID;                               // 情景ID
    char                szBrand[MAX_BRAND_NAME_LEN];            // 名称
...
typedef struct tagCFG_OIL_MASS_INFO
{
	...
	int					nTankHeight;			// 油箱高度，单位：毫米
	BOOL				bEnable;				// 异常油量事件上报使能
	int					nAbnormalOilLiter;		// 异常油量报警减少量，单位：0.1升。值扩大10倍，例如100表示10升，101表示10.1升
	int					nAbnormalOilTime;		// 常油量报警减少时间，单位：秒
	double				dbSpareHeight;			// 剩余油量高度，单位：毫米
	char				szReserved[1024];		// 预留字节
}CFG_OIL_MASS_INFO;
...
typedef enum tagEM_CFG_IDLEMOTION_FUNCTION
{
	...
	EM_CFG_IDLEMOTION_FUNCTION_AUTOPANGROUP,	// 水平旋转
...
typedef enum tagEM_CFG_OPEN_DOOR_GROUP_METHOD
{
    ...
    EM_CFG_OPEN_DOOR_GROUP_METHOD_FINGERPRINT,                  // 信息
    EM_CFG_OPEN_DOOR_GROUP_METHOD_ANY,                          // 任意组合方式开门
    EM_CFG_OPEN_DOOR_GROUP_METHOD_FACE,                         // 目标
...
typedef struct tagCFG_OPEN_DOOR_GROUP_DETAIL 
{
	...
	CFG_DOOR_OPEN_METHOD			emCombineMethod;					/// 多人开门支持任意组合开门方式 
	char							szReserved[1020];					/// 预留字段
...
typedef struct tagCFG_OPEN_DOOR_GROUP_INFO 
{
	...
	int						nGroupMaxNum;					// 组合总数，指明pGroupInfoEx实际内存空间大小，获取和下发均需要用户赋值,非0时nGroup、stuGroupInfo字段不生效
	int						nGroupRetNum;					// 实际有效的组合数，获取时由动态库赋值，下发时由用户赋值
	CFG_OPEN_DOOR_GROUP*			pGroupInfoEx; //多人开门组合信息扩展，内存有用户申请
	char						szReserved[2048];					// 保留字节
...
typedef struct tagCFG_OPEN_DOOR_ROUTE_INFO
{
	...
	UINT						nHolidayScheduleID;					// 假日计划, 需要反潜的假日计划
	BOOL						nClearCFG;							// 下发空配置，当该字段为TRUE，则配置中其余字段无效(仅下发有效)
	BOOL						bHolidayScheduleIDUnenable;			// 不下发假日计划，当该字段为TRUE，不下发假日计划(仅下发有效)
	char						szReserved[1012];					// 字节对齐
...
typedef enum tagCFG_THERMO_GAIN_MODE 
{
    ...
    CFG_THERMO_GAIN_MODE_HIGHTEMP,                      // 温度异常
    CFG_THERMO_GAIN_MODE_LOWTEMP,                       // 温度异常
...
{
    int                         nLowToHigh;             // 温度超过此设定值时，自动切换到温度异常模式
    int                         nLHROI;                 // 由温度异常切换到温度异常时的ROI 百分比0~100
    int                         nHighToLow;             // 温度下降到此设定值时，自动切换到温度异常模式
    int                         nHLROI;             	// 由温度异常切换到温度异常时的ROI 百分比0~100
...
///@brief 测温点报警设置Ex
typedef struct tagRADIOMETRY_ALARMSETTING_EX
{
	float						fPreThreshold;			// 预报警阈值,使用该字段需将bUsefPreThreshold置为TRUE
	UINT						nPreDuration;			// 预报警温度持续时间；单位：秒
	BOOL						bUsefPreThreshold;		// 预报警阈值使能
	char						byReserved[252];	    // 保留字节
} CFG_RADIOMETRY_ALARMSETTING_EX;
...
typedef struct tagRADIOMETRY_LOCALPARAM
{
	...
	int                         nRefalectedTemp;        // 目标反射温度（仅做兼容性保留。新对接使用fReflectedTemp字段）
...
///@brief 测温规则扩展
typedef struct tagCFG_RADIOMETRY_RULE_EX
{
	int								nBlackBodyTemp;			// 标定黑体温度，精度0.1，实际值扩大了10倍；比如显示是38度，实际该值获取的是380；如果需要设置39度，如该值需要传入390。
	BOOL							bUsefReflectedTemp;	    // 高精度目标反射温度使能
	float							fReflectedTemp;			// 目标反射温度，使用此字段需赋值bUsefReflectedTemp为true
	CFG_RADIOMETRY_ALARMSETTING_EX 	stuAlarmSettingEx[64];  // 测温点报警设置Ex，数组有效个数为CFG_RADIOMETRY_RULE.nAlarmSettingCnt
	float							fDistanceDecimalPart;	// 物体距离(小数部分)
	char							byReserved[240];	    // 保留字节
} CFG_RADIOMETRY_RULE_EX;
...
typedef struct tagCFG_RADIOMETRY_RULE_INFO
{
	...
	CFG_RADIOMETRY_RULE_EX      stRuleEx[512];			// 测温规则-扩展新增的字段
...
typedef struct tagCFG_THERMOMETRY_INFO
{
	...
	BOOL						bColdSpotFollow;			// 是否开启冷点探测追踪使能
	float						fDistanceDecimalPart;		// 物体距离(小数部分)
	char						szResvered[1016];			// 保留字节
...
typedef enum tagEM_CFG_RAINBRUSHMODE_MODE
{
	...
	EM_CFG_RAINBRUSHMODE_MODE_CYCLE,			// 周期模式
...
typedef struct tagCFG_GPS_INFO
{
    ...
    char                        szCurModule[CFG_COMMON_STRING_64];          // 当前模块类型标志
...
typedef struct tagCFG_PARKING_SPACE_LIGHT_GROUP_INFO_ALL
{
    ...
    CFG_PARKING_SPACE_LIGHT_GROUP_INFO          stuLightGroupInfoEx[16];        					// 车位指示灯本机配置扩展
    int                                         nCfgNumEx;                                          // 获取到的配置个数扩展
    BOOL										bIsUseLightGroupInfoEx;								// 使用扩展配置下发配置
    char										szResvered[508];									// 保留字节
...
typedef struct tagCFG_ACCESS_TEXTDISPLAY_INFO
{
	...
	UINT						nOpenDoorTipType;					// 开门提示类型, 0:未知, 1:仅提示成功失败, 2:用户信息(成功:工号,姓名,时间等,失败:失败原因,时间等), 3:用户信息,底库照片, 4:用户信息,抓拍照片,底库照片, 5:用户信息,抓拍照片 
	char						szResvered[1020];					// 保留字节
...
///@brief 加密配置信息,对应 CFG_CMD_NETNVR_ENCRYPT
...
///@brief 阻值范围
typedef struct tagCFG_RESISTANCE_INFO
{
	UINT					nHighValue;					// k欧姆（高）
	UINT					nLowValue;					// k欧姆（低）
	char					szReserved[64];				// 预留字节
}CFG_RESISTANCE_INFO;

///@brief ESD通用配置, 对应配置项目CFG_CMD_ESDCOMMSETTING
typedef struct tagCFG_ESDCOMMSETTING_INFO
{
	int						nStartCheckMode;			// 启动方式：1-人脸启动	2-刷卡启动
	CFG_RESISTANCE_INFO		stuHand;					// 手阻值范围
	CFG_RESISTANCE_INFO		stuLeftFoot;				// 左脚阻值范围
	CFG_RESISTANCE_INFO		stuRightFoot;				// 右脚阻值范围
	char					szReserved[1020];			// 预留字节
}CFG_ESDCOMMSETTING_INFO;

///@brief 门口机梯控信息配置
typedef struct tagCFG_CALL_LIFT_METHOD
{
	char										szCallLiftProtocol[32];				/// 呼梯协议, "Private": 私有, "RiBin": 日立, "TiBoShi": 梯博士
	int											nCallliftCmd;						/// 呼梯方式, 
																					/// 0: 未知
																					/// 1: 呼叫电梯到某个楼层，并开放其它任何楼层数
																					/// 2: 呼叫电梯到某个楼层，用户进去后只能按目标楼层
																					/// 3: 呼叫电梯到某个楼层后，还需要刷卡，电梯才能运行              
																					/// 4: 呼叫电梯到某个楼层,用户进去不用按钮，电梯会自动运行到目标楼层
	char										szReserved[60];						/// 保留字节
}CFG_CALL_LIFT_METHOD;

///@brief 门口机梯控信息配置
typedef struct tagCFG_NETWORK_LIFT_INFO
{
	char										szIP[40];							/// 网络梯控主机IP
	UINT										nPort;								/// 梯控主机端口号
	char										szName[32];							/// 梯控协议名称
	char										szReserved[60];						/// 保留字节
}CFG_NETWORK_LIFT_INFO;

///@brief 门口机梯控信息配置, 对应配置项目CFG_CMD_VTO_CALL_LIFT_INFO
typedef struct tagCFG_VTO_CALL_LIFT_INFO
{
	DWORD										dwSize;								/// 结构体大小
	BOOL										bIsCallLiftbyMainVTO;				/// 是否通过主门口机呼梯
	char										szMainVTOUsername[16];				/// 主门口机用户名
	char										szMainVTOPassword[32];				/// 主门口机密码
	char										szMainVTOIP[40];					/// 主门口机IP
	int											nCallLiftMethodCount;				/// 呼梯方式个数
	CFG_CALL_LIFT_METHOD						stuCallLiftMethod[16];				/// 呼梯方式
	int											nCustomCallliftCmd;					/// 呼梯方式
																					/// 0: 呼叫电梯到某个楼层，并开放其它任何楼层数
																					/// 1: 呼叫电梯到某个楼层，用户进去后只能按目标楼层
																					/// 2: 呼叫电梯到某个楼层后，还需要刷卡，电梯才能运行              
																					/// 3: 呼叫电梯到某个楼层,用户进去不用按钮，电梯会自动运行到目标楼层
	int											nScene;								/// 梯控设备使用场景, 0:梯箱外场景, 1:梯箱内场景
	int											nCallLiftPattern;					/// 呼梯方式, 0：主门口机呼梯, 1: 网络呼梯, 2: 本地呼梯
	int											nNetworkLiftInfoCount;				/// 网络梯控主机信息个数
	CFG_NETWORK_LIFT_INFO						stuNetworkLiftInfo[64];				/// 网络梯控主机信息
	int											nCombinationMode;					/// 呼梯组合方式
																					/// 0: 未知
																					/// 1: 仅呼叫第一部电梯
																					/// 2: 同时呼叫所有电梯
																					/// 3: 按奇偶房间号分别呼叫第一部或第2部电梯
	BOOL										bIsOpenDoorDelayCallLift;			/// 是否开启开门联动延时呼梯
	int											nDelayTime;							/// 延时呼梯时长，单位S，范围1-120
	int											nPublicFloorsCount;					/// 公共楼层列表个数
	char										szPublicFloors[256][8];				/// 公共楼层列表，表示支持的公共楼层
}CFG_VTO_CALL_LIFT_INFO;
...
{
	CFG_EM_PROTOCOL_PRIVATE3,                   // 私有3代协议
...
typedef struct tagCFG_LIGHTING_V2_UNIT
{
    ...
    UINT				nPercentOfMaxBrightness;			//当前白光灯上限亮度相对于白光灯最大亮度的百分比0~100
	int					nAIMixLightSwitchDelay;				//智能补光方案下生效，表示红外白光切换延时，防止来回切换影响寿命和体验单位秒 范围0-300 默认30s  
    BYTE                byReserved[120];			    	//预留字节		
...
CLIENT_CFG_API BOOL CALL_METHOD CLIENT_PacketIntelliSchemeData(char* szCommand, LPVOID lpInBuffer, DWORD dwInBufferSize, char* szOutBuffer, DWORD dwOutBufferSize);

///@brief 解析查询到的智能配置信息(szInBuffer，lpOutBuffer内存由用户申请释放)
CLIENT_CFG_API BOOL  CALL_METHOD CLIENT_ParseIntelliSchemeData(char* szCommand, char* szInBuffer, LPVOID lpOutBuffer, DWORD dwOutBufferSize, void* pReserved);
